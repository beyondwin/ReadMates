...
def _classify_name(path: str, violations: list[Violation]) -> int | None:
    name = Path(path).name
    if not name.endswith(".sql"):
        violations.append(Violation("catalog-non-sql", _public_path(path), "only .sql migrations are allowed"))
        return None
    match = MIGRATION_NAME_RE.fullmatch(name)
    if match is None:
        violations.append(
            Violation(
                "catalog-malformed-name",
                _public_path(path),
    ...
    if version == 0:
        violations.append(Violation("catalog-zero-version", _public_path(path), "migration versions must be positive"))
        return None
    return version
...
def _duplicate_violations(migrations: list[Migration]) -> list[Violation]:
...
def check_migrations(base_ref: str) -> int:
    ...
    merge_base_result = _run_git(repo, "merge-base", resolved_base.stdout.strip(), "HEAD")
    ...
    violations = base_violations + current_violations
...
class FlywayMigrationImmutabilityTests(unittest.TestCase):
    ...
    def _run_checker(self, repo: Path, base_ref: str) -> subprocess.CompletedProcess[str]:
        result = subprocess.run(
