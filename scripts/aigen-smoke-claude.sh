#!/usr/bin/env bash
# scripts/aigen-smoke-claude.sh — manual smoke for Claude (Anthropic) provider end-to-end
...
#
# Required env:
#   READMATES_AIGEN_ANTHROPIC_API_KEY  — Anthropic API key (live billing implications)
...
# including CLAUDE, and the model name below to be present in the YAML pricing
# table (default: `claude-sonnet-4-6`).
#
# OPERATOR CAVEAT: as of task_1_6 the live Anthropic SDK call in
# ClaudeApiClient.callTool throws NotImplementedError — the provider adapter
# is wired (FakeClaudeApi covers unit tests) but the live SDK invocation has
# not landed yet. This smoke script will therefore fail at the LLM call until
# the live-wiring task is merged. Run it post-merge.
#
# Live execution is currently waived (orchestrator directive: missing
# READMATES_AIGEN_ANTHROPIC_API_KEY in headless env); this script is the
# manual operator runbook for when the key is available post-merge.
set -euo pipefail
...
MODEL="${READMATES_SMOKE_CLAUDE_MODEL:-claude-sonnet-4-6}"
...
if [[ -z "${READMATES_AIGEN_ANTHROPIC_API_KEY:-}" ]]; then
  echo "READMATES_AIGEN_ANTHROPIC_API_KEY not set in environment of the *server* process; the smoke will 503 or fail at LLM call. Ensure the server has the key before running." >&2
fi
...
echo "✓ Claude smoke completed for jobId=$JOB_ID (model=$MODEL)"
