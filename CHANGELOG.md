...
- **참여 여정 중심의 내 공간:** 내 공간을 전체 참여 성취, 최근 참여 흐름, 다음 모임 nudge, 읽기 통계와 전체 책별 기록 진입을 갖춘 `나의 서재`로 개편했습니다.
...
- **멤버 내 공간 route 분리:** 전체 개인 기록과 계정 관리를 `/app/me/records`, `/app/me/settings`로 분리하고, 전역 계정 메뉴 로그아웃을 유지하면서 `/app/me`에도 하단 로그아웃을 제공합니다. 알림 수신 설정은 알림함 옆 `/app/notifications/settings`에서 관리합니다.
