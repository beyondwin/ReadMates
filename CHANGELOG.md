...
- **멤버 내 공간 route 분리:** 전체 개인 기록과 계정 관리를 `/app/me/records`, `/app/me/settings`로 분리하고, 전역 계정 메뉴 로그아웃을 유지합니다. `/app/me`는 계정 관리로만 연결하며, 알림 수신 설정은 알림함 옆 `/app/notifications/settings`에서 관리합니다.
