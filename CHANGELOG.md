...
- **호스트 생애주기 운영실:** 호스트의 canonical 업무 영역을 운영실(`/app/host`)·일정과 모임(`/app/host/sessions`)·사람(`/app/host/people`)·기록(`/app/host/records`) 네 곳으로 고정했습니다. 운영실은 서버가 고른 현재 모임을 준비실·현장·마감실로 나누고, 계산된 다음 행동 하나·준비 현황·호스트 작업함을 같은 문맥에 둡니다. 기존 `/members`, `/invitations`, `/operations`는 각각 `/people`, `/settings#invitations`, 운영실로 query/hash와 안전한 return state를 보존해 replace 이동하며, `/records`는 독립 canonical 기록 원장으로 유지합니다. 기존 모임 `/edit`·`/closing` deep link는 같은 모임 상세/기록 문맥으로 호환합니다.
