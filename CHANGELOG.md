...
- **멤버 내 공간 경로:** 전체 개인 기록과 계정 관리는 `/app/me/records`, `/app/me/settings`에서 제공하고, 로그아웃은 전역 계정 메뉴에 유지합니다. `/app/me`에는 페이지 로컬 로그아웃을 두지 않으며, 알림 수신 설정은 알림함 옆 `/app/notifications/settings`에서 관리합니다.
