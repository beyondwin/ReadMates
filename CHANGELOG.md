...
- **BFF 진단 라우트 `GET /api/bff/__internal/secret-status`**: configured secret count, rotation stage(`single`/`rotating`/`unconfigured`), primary secret의 SHA-256 첫 6자 fingerprint만 반환합니다 — raw secret 값은 노출하지 않습니다.
...
- **BFF 시크릿 rotation lifecycle 정착 (STRUCT-003)**: BFF 시크릿 rotation 단계와 운영 절차를 `ADR-0014`로 명시했습니다. `front/functions/_shared/proxy.ts`에 `getConfiguredBffSecrets`, `getRotationStage`, `secretFingerprint` helper를 추가하고, 새 진단 라우트 `GET /api/bff/__internal/secret-status`가 configured secret count, rotation stage(`single`/`rotating`/`unconfigured`), primary secret의 SHA-256 첫 6자 fingerprint만 반환합니다 — raw secret 값은 노출하지 않습니다.
- **배포 ledger NDJSON 스키마 (LOGIC-005)**: 배포 ledger의 권장 스키마를 `ADR-0016`으로 정착시키고, `05-deploy-compose-stack.sh`/`watch-compose-post-deploy.sh`의 `remote_ledger_append`가 기존 한 줄 포맷과 NDJSON `{ts, stage, event, status, detail:{...}}`를 동시에 기록합니다(기본 `READMATES_LEDGER_FORMAT=both`, `json`/`legacy`로 단일 포맷 선택 가능). watch 스크립트는 `trap watch_on_error ERR`로 예기치 못한 실패 경로에서도 ledger 이벤트를 보장하며, `01-vm-setup.sh`가 `jq`를 함께 설치합니다. 운영 런북에 jq 쿼리 예시와 env var 의미를 보강했습니다.
