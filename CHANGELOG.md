...
- **public page quality:** Public club, records, and session routes now derive document titles and meta descriptions from the same public club/session data used for visible pages, reducing local Lighthouse SEO metadata findings while leaving server API contracts, auth/BFF behavior, DB migrations, OAuth scopes, and deploy workflow behavior unchanged.
