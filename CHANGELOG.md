...
- Admin 승인 시안 01–07과 Host 승인 시안 07–17의 첫 화면 구성을 실제 로그인 화면으로 맞췄습니다. 구조 계약은 structurePass 12/6이라 구성 완료가 아닙니다. 2026-09-06 Jammy 재측정에서 승인 시안 대비 픽셀 비율 0.02는 18/18 미달(`not_passed_0.02`)이고 structurePass는 12 true / 6 false입니다. 이 상태로는 `origin/main` push가 시각 권위 job을 실패시킵니다. 사람 30초 이해도 검증과 보조기술 검증, 원격 CI는 `not measured`입니다.
...
- **모임 운영·기록 경로:** 모임 주소는 `/app/host/sessions/:sessionId`이고 `/edit`와 `/closing`은 그 화면으로 이동합니다. 기록 장부·과거 검색은 canonical 기록 원장(`/records`)에서 제공합니다. 다음 책은 여러 권 미리 넣고 멤버에게 보일 수 있으며, 기록은 정리본 파일로 올립니다. 새 모임은 `GET /api/host/sessions/schedule-defaults`로 최근 일정을 채우고, 내구 이력이 없는 `DRAFT`는 목록에서 휴지통으로 옮길 수 있습니다.
...
- **관리자 명령 key rotation·운영 안전성:** admin digest previous key는 overlap에서 lookup·dual-write하고 writer drain 뒤 current-only alias write로 전환합니다. Claim·replay·purge·retirement는 digest key-state를 같은 순서로 잠그며, startup은 referenced key 누락·unknown version·24시간 미만 retirement evidence·DB 조회 실패를 readiness 전에 거부합니다. Metric tag는 allowlisted command/result/outcome만 사용합니다.
...
- **Host-client v3 release gate:** 모든 mutating `/api/host/**`에 global v3 generation을 적용하고 R1 support → R2a safety/cache → R2b browser v3 → 24시간 residue-zero → R3 enforcement 순서를 structural checker와 attested evidence contract로 고정했습니다. Repository 검증은 artifact-ready까지만 증명하며 live stage는 각각 fresh approval이 필요합니다.
- **권한 상실 client purge:** `HOST_AUTHORITY_REVOKED`, `MEMBERSHIP_SUSPENDED`, `CROSS_CLUB_SCOPE`에서 해당 club의 host query/mutation/draft/return state를 취소·폐기하고 safe route로 replace합니다. Recoverable revision conflict와 response loss는 입력을 보존합니다.
...
- **플랫폼 어드민 review 계약 정합성:** audit pagination은 filter·snapshot·source set에 결속된 V57 purpose-separated cursor를 current key로만 발급하고 rotation 중 previous key 검증을 제한적으로 허용합니다. Analytics frontend는 기존 v2 서버와 새 additive KPI 필드의 배포 순서 차이를 안전하게 정규화하며, delta가 없는 응답을 임의 재계산해 운영자에게 표시하지 않습니다.
...
- **Flyway V57:** raw idempotency key·canonical request·reason·email을 저장하지 않는 platform-admin command claim, per-key-version HMAC alias, digest-key retirement state를 additive하게 추가합니다. Actor·target·receipt에는 deletable domain FK를 두지 않고 operational alias만 claim 삭제에 cascade합니다.
- **Flyway V58–V60:** V58은 club metadata·visibility·onboarding command receipt와 invitation/domain convergence target을 추가합니다. V59는 기존 notification confirmation을 보존·backfill하면서 destructive FK를 snapshot semantics로 교체하고 notification·AI typed receipt와 append-only provider attempt evidence를 추가합니다. V60은 support grant preview/receipt와 active-slot uniqueness를 추가하고, active 중복은 `(created_at, id)` 역순의 최신 한 건만 유지해 나머지를 revoke하며 count-only migration evidence를 남깁니다. Legacy free-text reason은 복구 불가능한 `LEGACY_UNCLASSIFIED + notePresent` evidence로 즉시 redaction합니다. 적용된 migration은 수정·삭제하지 않고 더 높은 version의 forward-fix로만 보완합니다.
- **Flyway V61–V65:** V61은 session schedule revision, participant별 최소 seen fact와 coarse ACTIVE club access를 추가하고, V62는 host-owned deferral과 15분 immutable workbox snapshot/page를 추가합니다. V63은 editable manual notification copy와 대상·일정 revision snapshot을 preview/dispatch에 보존하고, V64는 raw token을 저장하지 않는 named host invitation link와 OAuth consumption evidence를 추가합니다. V65는 club settings/co-host revision history와 운영 종료 preview/idempotency receipt를 추가합니다. 모두 forward-only이며 적용된 SQL을 수정·삭제하지 않습니다.

### Deployment Notes

- Backend startup보다 먼저 GitHub Secrets에 host cursor·host mutation·admin command의 세 current HMAC key를 provision하고 version Variables를 확인한 뒤 `sync-config(restart_api=false, dry_run=false)`로 env를 렌더링합니다. 그 다음 같은 tag의 backend를 배포해 forward-only Flyway V52–V60과 health를 확인하고, 마지막에 호환되는 Pages BFF/frontend를 배포합니다. 이 순서를 지킬 수 없으면 backend promotion을 시작하지 않습니다.
...
- V52–V60은 forward-only migration입니다. V52–V58은 schema를 additive하게 확장하고, V59는 기존 notification confirmation을 보존하면서 destructive FK를 snapshot semantics로 교체하는 compatibility migration입니다. V60은 legacy support free-text를 의도적으로 비가역 redaction하므로 적용 전 support write traffic을 drain합니다. Backend health 확인 직후 같은 tag의 Pages BFF/frontend를 배포한 다음 traffic을 복구합니다. 중간 혼합 구간에서 legacy support create/revoke는 `410`으로 fail closed하며 category를 추정하거나 auto-confirm하지 않습니다. 회귀 시 적용 migration을 수정·삭제하거나 tag를 이동하지 않고 schema와 immutable evidence를 보존한 compatible image 또는 더 높은 version의 forward-fix migration을 사용합니다.
- V61–V65는 backend Flyway가 먼저 적용해야 합니다. 배포 전 DB backup/snapshot을 확보하고, backend 시작 뒤 `flyway_schema_history`의 V61–V65 성공과 health를 확인한 다음 compatible frontend를 배포합니다. 회귀 시 적용 migration을 수정·삭제하거나 downgrade하지 않고 schema와 immutable snapshot/receipt/history를 읽을 수 있는 compatible image 또는 더 높은 version의 forward-fix migration을 사용합니다. 이 항목은 배포 순서이며 실제 production rollout 완료를 뜻하지 않습니다.

### Database

- **Flyway V49–V55:** lifecycle audit, 변경 snapshot, 7일 휴지통, domain revision/participant snapshot, host idempotency receipt, public convergence ledger, emergency takedown preview/receipt를 additive하게 추가합니다. 기존 migration은 수정하지 않으며 rollback은 schema와 immutable audit bytes를 보존한 compatible image 또는 더 높은 forward-fix migration을 사용합니다.
...
- **AI Kafka·Redis 복구 가용성:** generic Kafka exhaustion은 총 10회 뒤 durable terminal/no-op 또는 고정 unroutable 결과일 때만 offset을 commit하고, restart scheduler가 deadline을 지난 `PENDING`/`RUNNING`을 같은 원자적 Redis 전이로 복구합니다. Receipt/window token이 소유한 PENDING charge만 환불하며 RUNNING provider/cost evidence는 보존합니다. Persisted bounded index repair와 sampled queue probe는 현재 epoch full pass·ceiling·quarantine을 확인해 실제 0과 unavailable `NaN`을 구분하고, 고정 recovery/repair metric·Prometheus alert·Grafana/runbook을 제공합니다. DLT, live provider 호출, DB/API/frontend 변경은 없습니다.

- **백엔드 품질 래칫:** 프로덕션 Kotlin 경고를 0건으로 만들고 신규 경고를 빌드 오류로 처리합니다. Detekt·ktlint baseline, JaCoCo 43%, 전체 인바운드 슬라이스 registry, 기존 경계 import와 기능 의존 edge를 단방향 gate로 고정해 새 품질·아키텍처 부채가 추가되지 않도록 했습니다. Phase 2 완료 시점의 boundary partition은 `0 current + 39 retired = 39 approved`, feature partition은 `37 + 4 = 41`이고 cyclic component는 0개입니다. 정적 분석은 전역 zero가 아니며 비대상 legacy debt로 Detekt `437 current + 24 retired = 461 approved`, ktlint `171 + 0 = 171`을 명시적으로 유지합니다.
- **Actor/auth–club 경계 정리:** 식별자와 capability만 가진 순수 `ClubActor`·`PlatformActor`를 application authorization input으로 도입하고, 기존 principal은 inbound carrier로 유지합니다. Auth가 HTTP club context를 소유하고 servlet-security/OAuth handler는 input port와 application model만 주입받으며, club-owned invitation-token port로 `club -> auth` 역방향을 제거했습니다. 이 단계의 중간 ledger `4/35/39`, `40/1/41`은 이후 session-family 전환과 책임 분해를 거쳐 위 최종 partition으로 닫혔습니다. REST/API shape, authorization 의미, BFF/frontend source, migration/schema, deployment와 production behavior는 바꾸지 않았습니다.
- **Session-family ownership과 cycle 제거:** sessionclosing parsing과 sessionimport failure를 각 inbound/application 소유로 옮기고, sessionrecord가 history sort, visibility/history cursor, snapshot codec output port, live replacement output port를 소유하도록 정리했습니다. Sessionimport가 record-owned replacement port를 구현하며 바깥 apply transaction, immutable revision, notification/cache 의미를 유지합니다. 네 boundary tombstone으로 ledger는 `0/39/39`, `sessionrecord|sessionimport`·`sessionrecord|session`·`aigen|session` tombstone으로 feature ledger는 `37/4/41`이고 cycle은 0개입니다. `sessionimport|sessionrecord`·`session|sessionrecord` 순방향과 REST/JSON/status/error/cursor 계약은 유지하며, frontend/BFF, migration/schema, deploy, production behavior는 바꾸지 않았습니다.
...
- **기본 로그인 진입:** 별도 복귀 경로가 없는 `/login`도 읽는사이 클럽의 `둘러보기`와 `멤버로 시작`을 제공해 공개 화면과 같은 선택지를 유지합니다. 명시된 안전한 `returnTo`는 기본값보다 우선하며, 오류 복구와 안전하지 않은 외부 복귀 경로는 암묵적인 클럽 가입 문맥을 만들지 않습니다.
...
- **반응형 계정·내 공간·북클럽 아바타:** 모바일과 데스크톱에서 계정 접근과 호스트 공간 전환을 명시적으로 구분하고, 알림·계정 설정에 고정된 `내 공간` 상위 동선을 제공합니다. 멤버 식별은 외부 프로필 사진 대신 privacy-safe 로컬 book-club 아바타 30종을 사용하며, 미사용 아바타 우선 자동 배정 후에도 멤버가 `내 공간`의 통합 편집기에서 표시 이름과 함께 변경할 수 있습니다. 내 공간·편집기·내비게이션·멤버 목록·작성자 표시는 역할별 의미 크기로 정규화되고, 선택기는 30개 이름을 항상 보이면서 현재 선택을 체크 배지 없이 테두리와 절제된 배경으로 구분하고 키보드 focus outline을 별도로 유지합니다.
...
- **공개 릴리스 안전:** TypeScript incremental build metadata(`*.tsbuildinfo`)를 공개 릴리스 후보에서 제외하고 검사기와 fixture로 로컬 빌드 경로 재유입을 차단합니다. 후보에는 Node/pnpm 루트 계약과 frontend가 참조하는 `design/system`, CI가 검증하는 `design/docs`를 함께 포함해 저장소 밖에서도 frozen install과 build를 재현할 수 있게 했습니다.
- **AI admission/API 경계:** 비용 admission을 Redis Lua로 원자화하고 AI 전용 분당 한도와 fail-closed 동작을 적용했습니다. Retry/repair/regeneration을 포함한 각 provider network call 직전에 5분 owner lease를 갱신하고 SDK timeout을 4분으로 제한하며 provider SDK 내부 retry를 꺼, queue 지연·stale worker·숨은 network attempt가 동시 비용 및 job call cap을 우회하지 못합니다. Problem 응답은 RFC 7807 media type을 사용하며, browser draft는 고정 6시간 TTL/예약 삭제/만료 후 재저장 차단으로 제한되고 BFF는 과대 multipart body를 buffering 전에 거절합니다. Frontend Zod fixture와 server 직렬화 계약 검증이 model default, job/evidence, regeneration, commit receipt, problem shape drift를 막습니다.
...
- 기존 SEND/SKIP mutation 계약을 제거한 새 frontend/server API 조합은 이전 frontend 또는 server와 host write를 혼용할 수 없으므로 `v2.0.0` tag push는 `Deploy Server Image`만 시작합니다. Scan/promote 뒤 `sync-config(restart_api=false, dry_run=false)`가 `READMATES_HOST_WRITE_CLIENT_CONTRACT_REQUIRED=true`를 포함한 fail-closed runtime을 준비하고, GHCR image를 OCI Compose에 반영해 V37–V42 Flyway 성공과 backend health를 확인합니다. 이 시점부터 구 Pages BFF의 host mutation은 409로 잠시 동결됩니다. 마지막으로 `Deploy Front`를 `release_tag=v2.0.0`으로 수동 실행해 같은 tag의 browser bundle과 Pages Functions를 함께 배포하면 v2 handshake가 성립해 host write가 재개됩니다.
