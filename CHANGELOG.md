...
- **나의 서재 중심의 내 공간:** 내 공간을 정확한 개인 독서 통계와 최근 세 권의 회차별 기록을 우선하는 `나의 서재`로 개편했습니다.
...
- **멤버 내 공간 route 분리:** 전체 개인 기록과 계정 관리를 `/app/me/records`, `/app/me/settings`로 분리하고, 로그아웃을 전역 계정 메뉴로 옮겼습니다. 알림 수신 설정은 알림함 옆 `/app/notifications/settings`에서 관리합니다.
