...
- **모임 운영 장부:** 호스트 홈이 지금 다루는 모임의 모임 전·진행 중·모임 후 장부입니다. 다음 책은 여러 권 미리 넣고 멤버에게 보일 수 있으며, 기록은 정리본 파일로 올립니다. 모임 주소는 `/app/host/sessions/:sessionId`이고 `/edit`와 `/closing`은 그 장부로 이동합니다. 새 모임은 `GET /api/host/sessions/schedule-defaults`로 최근 일정을 채우고, 내구 이력이 없는 `DRAFT`는 목록에서 지울 수 있습니다.
