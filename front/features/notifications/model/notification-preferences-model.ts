...
> = {
  NEXT_BOOK_PUBLISHED: { label: "다음 책 공개", sub: "예정 모임이 멤버에게 열릴 때" },
  SESSION_REMINDER_DUE: { label: "모임 전날 리마인더", sub: "모임 하루 전 준비 알림" },
  FEEDBACK_DOCUMENT_PUBLISHED: { label: "피드백 문서 등록", sub: "참석한 모임의 피드백 문서가 올라올 때" },
  REVIEW_PUBLISHED: { label: "다른 멤버의 서평 공개", sub: "발행된 모임에 새 공개 서평이 올라올 때" },
};
