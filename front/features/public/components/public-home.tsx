...
function LatestRecordFeature({ session }: { session: PublicSessionListItem | null }) {
  if (!session) {
...
function ArchiveRecordRow({ session }: { session: PublicSessionListItem }) {
  const display = sessionDisplay(session);
...
function SummaryExcerpt({ session }: { session: PublicSessionListItem }) {
  const display = sessionDisplay(session);
