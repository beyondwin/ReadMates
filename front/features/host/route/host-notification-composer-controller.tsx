...
export function HostNotificationComposerController({
  ...
  return (
    <OpenComposerController
      key={`${request.sessionId}:${request.eventType}:${request.origin}`}
      request={request}
      context={context}
      onClose={onClose}
...
function OpenComposerController({
  ...
  const unavailable = optionsQuery.isError || (optionsQuery.data && !template);
  return (
...
function ReadyComposerController({
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      setError("미리보기를 만들지 못했습니다. 대상과 채널을 확인해 주세요.");
    }
  };
  ...
  const handleConfirm = async (resendConfirmed: boolean) => {
    ...
    try {
      ...
      setError("발송을 요청하지 못했습니다. 미리보기 만료 또는 재발송 여부를 확인해 주세요.");
    }
  };
