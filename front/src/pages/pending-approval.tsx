...
import type { ApprovalState, AuthMeResponse, MembershipStatus } from "@/shared/api/readmates";
...
function copyForAuth(auth: AuthMeResponse): PendingStateCopy {
...
export default function PendingApprovalPage() {
  ...
  const email = state.auth.email ?? "Google 계정";
        ...
        <div className="container">
          <div className="surface auth-card auth-card--pending">
            ...
            <div className="auth-boundary-list" aria-label="현재 계정 승인 상태">
              ...
              <div className="auth-boundary-row">
                <span>멤버 상태</span>
                <strong>{state.auth.membershipStatus ?? "확인 필요"}</strong>
              </div>
              <div className="auth-boundary-row">
                <span>승인 상태</span>
                <strong>{state.auth.approvalState}</strong>
                <em>{copy.note}</em>
              </div>
            </div>
