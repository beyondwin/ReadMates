...
export type ReadmatesDataState<T> =
  | { status: "loading" }
  | { status: "ready"; data: T }
  | { status: "error"; error: Error };

export function requestReadmatesRouteRefresh() {
  if (typeof window !== "undefined") {
    window.dispatchEvent(new Event(READMATES_ROUTE_REFRESH_EVENT));
  }
}

export function useReadmatesData<T>(load: () => Promise<T>): ReadmatesDataState<T> {
  const [state, setState] = useState<ReadmatesDataState<T>>({ status: "loading" });
