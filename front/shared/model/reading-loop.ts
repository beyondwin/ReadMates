...
export const READING_LOOP_LABELS: Record<ReadingLoopState, string> = {
  ...
  SESSION_READY: "모임 준비됨",
  REFLECTION_DUE: "회고 필요",
  ARCHIVE_AVAILABLE: "아카이브 연결",
};

const READING_LOOP_DESCRIPTIONS: Record<ReadingLoopState, string> = {
  NO_SESSION: "현재 열린 모임이 없어 호스트가 새 모임을 열면 멤버 준비가 시작됩니다.",
  HOST_SETUP_REQUIRED: "호스트가 모임 정보, 멤버 상태, 공개 범위, 운영 대기 항목을 먼저 닫아야 합니다.",
  MEMBER_PREP_REQUIRED: "멤버가 참석 응답, 읽기 진행률, 질문 중 남은 준비를 완료해야 합니다.",
...
export function getReadingLoopNextAction(input: ReadingLoopNextActionInput): ReadingLoopNextAction {
  ...
  if (input.state === "MEMBER_PREP_REQUIRED") {
    switch (input.missing) {
      case "RSVP":
        return { label: "참석 응답하기", href: "/app/session/current", target: "current-session" };
      case "CHECKIN":
      ...
      case "REFLECTION":
      case "ARCHIVE":
        return { label: "모임 열기", href: "/app/session/current", target: "current-session" };
    }
  }
