...
describe("MemberHome", () => {
  ...
  it("shows viewer members a read-only notice on member home", () => {
    ...
    expect(desktop.getByText("전체 세션은 볼 수 있어요. 정식 멤버가 되면 RSVP, 읽기 진행률, 질문 작성이 열립니다.")).toBeInTheDocument();
    expect(mobile.getAllByText("둘러보기 멤버").length).toBeGreaterThan(0);
    expect(mobile.getAllByText("전체 세션은 볼 수 있어요. 정식 멤버가 되면 RSVP, 읽기 진행률, 질문 작성이 열립니다.").length).toBeGreaterThan(0);
