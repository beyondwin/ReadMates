...
| 근거 기반 AI 세션 생성과 외부 JSON 가져오기 형식을 확인한다 | [`development/session-import-generator.md`](development/session-import-generator.md) |
| 디자인 시스템과 gallery catalog를 확인한다 | [`../design/README.md`](../design/README.md) |
| 운영에 배포하거나 배포 절차를 확인한다 | [`deploy/README.md`](deploy/README.md) |
| 배포 attempt, 진단, 관측성, 사후 보고 운영 절차를 본다 | [`operations/README.md`](operations/README.md) |
| 어떤 surface에 무엇을 쓸지 정해서 작업한다 | 루트 [`AGENTS.md`](../AGENTS.md) → [`agents/`](agents) 가이드 |
| 비자명한 문제의 설계·운영 검증 deep-dive를 읽는다 | [`case-studies/README.md`](case-studies/README.md) |
...
- [`agents/`](agents) — 공통 실행 계약과 surface별 작업 가이드(front, server, design, docs). 루트 [`AGENTS.md`](../AGENTS.md)가 router, 이 디렉터리가 본문입니다.
- [`case-studies/`](case-studies) — 비자명한 문제의 *문제 → 접근 → 구현 → 검증 → trade-off* deep-dive. ADR이 결정 카드라면 case study는 그 결정에 도달한 사고 과정과 운영 검증입니다.
