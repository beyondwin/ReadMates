...
`front/src/app/router.tsx`는 composition root입니다. Route 정의는 `front/src/app/routes/{public,auth,member,host,admin}.tsx`에 variant별로 분리되어 있으니, 새 route는 해당 variant 모듈에 추가합니다.
...
Place new feature code in `api`, `queries`, `model`, `route`, or `ui`; do not reintroduce a feature-level `components` directory.
...
When local `pnpm` differs from the pinned `packageManager`, run the same commands through Corepack as described in the root `AGENTS.md` and report the exact command.
