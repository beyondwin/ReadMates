...
- 기존보다 넓은 1040~1120px 범위의 지면을 사용하되 viewport 가장자리 여백을 보존한다.
...
- 빈 책 제목: 링크의 접근 가능한 이름과 fallback이 비지 않도록 기존 API contract validation 또는 안전한 `책` fallback을 사용한다.
...
- 페이지에 하나의 `h1`을 유지하고 최근 기록은 `h2`, 각 책 제목은 목록 문맥 안의 적절한 heading 또는 strong text로 구성한다.
- 프로필과 성취는 각각 접근 가능한 이름을 갖되 하나의 상위 overview section 안에 포함한다.
- 최근 기록은 semantic list이며 각 행은 하나의 명확한 anchor다.
- 표지 이미지의 alt는 행의 책 제목을 불필요하게 반복하지 않도록 장식 처리하거나 현재 접근 가능한 이름 계약과 중복되지 않게 결정한다.
