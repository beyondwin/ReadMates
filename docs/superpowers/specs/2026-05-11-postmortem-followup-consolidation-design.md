...
1. **Post-mortem 본문 갱신** — 본 라운드의 R2/R3 재평가 결과를 incident post-mortem의 *Action items* 표에 반영. P3=closed (grep으로 검증됨), P2=ADR-0011 spec link 추가.
...
- **#2 (P2)** — 별도 spec `2026-05-11-bff-host-header-policy-design.md`(ADR-0011)에서 처리. **트래킹 = 그 spec link**.
...
| 2 | BFF host 헤더 정책 ADR 후속 작성 — shared fallback domain일 때 host 헤더 미전송 | P2 | docs | In progress | `docs/superpowers/specs/2026-05-11-bff-host-header-policy-design.md` (ADR-0011 후보) |
...
| 일자 | 변경 | 출처 |
|------|------|------|
| 2026-05-11 | Action item #3 → Closed, #2 → ADR-0011 spec link, #1 → Deferred | `docs/superpowers/specs/2026-05-11-postmortem-followup-consolidation-design.md` |
```
...
- ADR-0011 본문 작성 — 별도 spec/plan(`2026-05-11-bff-host-header-policy-design`)에서.
...
| 1~5번을 1쌍에 ADR-0011까지 통합 | ADR-0011은 *결정*이고 본 spec은 *후속*. 청중과 톤이 다름. |
...
- ADR-0011 작성 후, parity test (P1)의 시급성 재평가 — 필요 시 별도 spec.
