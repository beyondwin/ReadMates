...
- `archive`, `feedback`, `public` read path는 아직 Query migration 후속 후보로 남아 있다.
- 호스트 AI commit 성공 후 세션 편집기는 `window.location.reload()`로 server state를 회복한다.
- AI generation의 durable audit row와 Redis live job state는 존재하지만, 운영자가 전체 job 상태, 실패 코드, 비용, stale 후보를 보는 `/admin` 표면은 없다.
- 호스트는 새로고침하거나 화면을 떠났다가 돌아온 뒤 자기 세션의 in-flight/recoverable job을 다시 찾는 전용 read path가 없다.
...
현재 host session editor의 AI commit success callback은 full reload를 사용한다. vNext에서는 다음으로 바꾼다.
...
- `SUCCEEDED`
- `COMMITTING`
- `FAILED` 중 retry 안내가 필요한 safe error
...
- provider/model별 token/cost aggregate
- failure-code 분포
...
- MySQL historical row만 있고 Redis live job이 없는 경우에는 no-op 성공이 아니라 `JOB_EXPIRED` 또는 `JOB_NOT_LIVE` 성격의 safe error를 반환한다.
...
`JobRecord` 또는 Redis hash에는 safe `createdAt`/`lastUpdatedAt` metadata를 추가한다. `AiGenerationJobStore.save`, `transitionStatus`, `deleteTransientPayload`, `delete`는 index와 `lastUpdatedAt`을 함께 갱신해야 한다. 상태가 terminal로 바뀌면 active index에서 제거하고, session recent index는 TTL 동안 최근 job lookup이 가능하도록 유지한다.
