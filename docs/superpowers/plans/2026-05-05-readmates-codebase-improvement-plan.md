...
**배경:** root `compose.yml`의 `readmates`/`<ci-mysql-password>` 값은 로컬 개발용 public-safe sample이라 운영 secret 유출은 아닙니다. 그래도 `.env` override를 지원하면 로컬 환경 충돌을 줄이고 public repo safety policy와 일관성이 좋아집니다.
