...
- [x] Remove or generalize local absolute paths such as `<local-user-path> from public docs.
...
- `git ls-files .env.local front/.env.local .env front/.env deploy/oci/.deploy-state` returned no files.
- `rg -n '<local-user-path> Actions.*secret|github actions.*secret' README.md .env.example docs/deploy deploy/oci/02-configure.sh deploy/oci/03-deploy.sh || true` returned no matches during remediation.
...
- The fallback content scanner now emits matched substrings with `git grep -o`, `rg -o`, or `grep -o`, then allowlists only the matched assignment value. A synthetic datasource password assignment fixture fails because placeholder comment text remains outside the matched value.
...
- Updated fixture coverage so synthetic datasource password assignment fixtures fail, and both placeholder and environment-variable indirection fixtures pass.
...
- Built the public candidate with `./scripts/build-public-release-candidate.sh`; exit code 0. The script printed the candidate path as `<local-user-path> and the next verification commands.
...
```bash
rg -n 'wooseung|seoyun|readmates\.kr|gmail\.com|<local-user-path> .tmp/public-release-candidate || true
```
...
```bash
rg -n '<local-user-path> .tmp/public-release-candidate || true
```

The only output was intentional documentation/script text for `.tmp/public-release-candidate` and the checker's split local-path pattern string; no concrete `<local-user-path> local path was present in the candidate.
...
- Confirmed the accidental patch target issue was reverted in `<local-user-path> `git status --short -- README.md docs/deploy/README.md` returned no output there.
- `rg -n 'docs/superpowers|<local-user-path> README.md docs/deploy/README.md || true` returned no output.
...
```text
> readmates-front@ lint <local-user-path>
> eslint .
...
- Ran the requested final verification commands from `<local-user-path> only. No publish, push, commit, remote, visibility, secret-rotation, candidate Git init, or other worktree operation was performed.
...
- `rg -n 'docs/superpowers|wooseung|seoyun|readmates\.kr|gmail\.com|<local-user-path> .tmp/public-release-candidate || true` exited 0 with no stdout.
...
- The parent private source worktrees at `<local-user-path> and `<local-user-path> now use `https://github.com/beyondwin/ReadMates-legacy.git` as fetch origin and keep push URL disabled as `DISABLED_BY_CODEX_NO_PUSH_APPROVAL`.
...
ReadMates public-release check
  mode: candidate
  source: <local-user-path>
	Running gitleaks dir <local-user-path>
