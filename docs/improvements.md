...
- **(DX P2)** README/agents 문서는 풍부하지만 `/Users/kws/...` 같은 cwd 의존이 reset되는 환경 외에서도 안전하게 동작하도록 한 곳에서 통합된 `Makefile` 또는 `Justfile`가 없고, 단일 명령어로 `lint+test+build+e2e` 통합 실행을 제공하지 않는다. Server는 Gradle alias도 없다.
...
- **(Spring Boot dev 경험)** `bootRun`은 cli command 길다. `server/.tmp/run-dev.sh`(또는 `scripts/run-server-dev.sh`)를 두고 README env 묶음을 그 안에 두면 onboarding curve가 줄어든다. `READMATES_BFF_SECRET=local-dev-secret` 같은 dev placeholder는 `.gitleaks.toml` allowlist에 이미 들어가 있으므로 안전.
...
- **본 보고서가 참조한 위치**(절대 경로):
  - 보안 리포트: `/Users/kws/source/web/ReadMates/.gstack/security-reports/2026-04-27-194155+0900-readmates-security-posture.md`
  - 아키텍처 SoT: `/Users/kws/source/web/ReadMates/docs/development/architecture.md`
  - 서버 entry: `/Users/kws/source/web/ReadMates/server/build.gradle.kts`, `/Users/kws/source/web/ReadMates/server/src/main/resources/application.yml`
  - 보안 wiring: `/Users/kws/source/web/ReadMates/server/src/main/kotlin/com/readmates/auth/infrastructure/security/SecurityConfig.kt`, `BffSecretFilter.kt`, `RateLimitFilter.kt`
  - 거대 파일 후보:
    - `/Users/kws/source/web/ReadMates/server/src/main/kotlin/com/readmates/auth/adapter/out/persistence/JdbcMemberAccountAdapter.kt` (612 LOC)
    - `/Users/kws/source/web/ReadMates/server/src/main/kotlin/com/readmates/note/adapter/out/persistence/JdbcNotesFeedAdapter.kt` (576)
    - `/Users/kws/source/web/ReadMates/server/src/main/kotlin/com/readmates/notification/adapter/out/persistence/NotificationDeliveryQueries.kt` (574)
    - `/Users/kws/source/web/ReadMates/server/src/main/kotlin/com/readmates/session/adapter/out/persistence/HostSessionWriteOperations.kt` (540)
    - `/Users/kws/source/web/ReadMates/front/features/archive/ui/archive-page.tsx` (1074)
    - `/Users/kws/source/web/ReadMates/front/features/host/ui/host-session-editor.tsx` (857)
    - `/Users/kws/source/web/ReadMates/front/features/host/ui/host-notifications-page.tsx` (856)
    - `/Users/kws/source/web/ReadMates/front/features/current-session/ui/current-session-mobile.tsx` (798)
  - CI 워크플로우: `/Users/kws/source/web/ReadMates/.github/workflows/{ci,deploy-front,deploy-server}.yml`
  - BFF: `/Users/kws/source/web/ReadMates/front/functions/api/bff/[[path]].ts`, `/Users/kws/source/web/ReadMates/front/functions/_shared/proxy.ts`
  - DB 마이그레이션: `/Users/kws/source/web/ReadMates/server/src/main/resources/db/mysql/migration/` (V1, V9~V22)
  - `.gitleaks.toml`: `/Users/kws/source/web/ReadMates/.gitleaks.toml`
