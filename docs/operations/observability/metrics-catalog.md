...
|------|------|------|------|------|----------|------|------|
| `readmates.notifications.outbox.backlog` | gauge | `status` (`pending` / `failed` / `dead` / `publishing`) | 행수 | status별 `notification_event_outbox` 적체. 첫 refresh 전에는 `NaN`이며, 조회 실패는 마지막 성공 snapshot을 보존한다. | `server/.../ReadmatesOperationalMetrics.kt`, `server/.../CachedNotificationBacklogProvider.kt` | dashboards.md#notification-pipeline | [backlog](alerts.md#notificationoutboxbackloghigh), [dead](alerts.md#notificationoutboxrelaydead) |
| `readmates.notifications.delivery.backlog` | gauge | `status` (`pending` / `failed` / `dead` / `sending`) | 행수 | status별 `notification_deliveries` 적체. 첫 refresh 전에는 `NaN`이며, 조회 실패는 마지막 성공 snapshot을 보존한다. | `server/.../ReadmatesOperationalMetrics.kt` | dashboards.md#notification-pipeline | [backlog](alerts.md#notificationdeliverybackloghigh) |
| `readmates.notifications.backlog.refresh` | counter | `result` (`success` / `partial` / `failure`) | 건수 | event/delivery backlog refresh 결과. 한쪽만 성공하면 `partial`; 실패한 쪽은 마지막 성공 snapshot을 유지한다. | `server/.../NotificationBacklogRefreshScheduler.kt`, `server/.../CachedNotificationBacklogProvider.kt` | dashboards.md#notification-pipeline | — |
| `readmates.outbox.publish` | counter | `result` (`success` / `failure` / `dead` / `missing_payload` / `expired` / `stale_lease`) | 건수 | relay 시도 결과. `success`/`failure`/`dead`/`missing_payload`/`expired`는 해당 DB transition이 commit된 뒤 기록하고, `stale_lease`는 exact-lease CAS가 거절되어 어떤 transition도 commit되지 않았음을 기록한다. raw provider 오류나 event ID는 label에 넣지 않는다. | `server/.../NotificationRelayService.kt` | dashboards.md#notification-pipeline | slos.md#notification_dispatch_success_ratio |
...
| `readmates.admin.health.provider.outcomes` | counter | `provider` (8 fixed card IDs), `result` (`SUCCESS` / `ERROR` / `TIMEOUT` / `REJECTED`) | 건수 | 실제 refresh wave의 provider invocation 결과. card payload, exception message, URL을 label로 사용하지 않는다. | `server/.../admin/health/adapter/out/observability/MicrometerPlatformAdminHealthMetricsAdapter.kt` | — | — |
...
| `readmates.aigen.commit.recovery.failures` | counter | (없음) | 건수 | Receipt 확인 또는 commit 상태 복구 시도가 실패한 횟수. Content나 row-level ID를 label로 사용하지 않는다. | `server/.../aigen/application/service/AiGenerationMetrics.kt` | — | — |
| `readmates.aigen.failure.recovery` | counter | `source` (`kafka` / `scheduled`), `result` (고정 recovery enum) | 건수 | Kafka exhaustion 또는 deadline scheduler가 분류한 최종 job 복구 결과. Application service가 job별 정확히 한 번 기록하며 `failed`/`corrupt`는 운영 검토 대상이다. | `server/.../aigen/application/service/AiGenerationMetrics.kt` | dashboards.md#ai-session-generation | alerts.md#aigenrecoveryfailure |
| `readmates.aigen.recovery.index.repair` | counter | `result` (`page_completed` / `pass_completed` / `epoch_reset` / `quarantined` / `over_cap` / `failed`) | 건수 | Persisted processing-index repair page 결과. 한 page는 `failed > over_cap > epoch_reset > quarantined > pass_completed > page_completed` 우선순위로 하나만 기록한다. | `server/.../aigen/application/service/AiGenerationMetrics.kt` | dashboards.md#ai-session-generation | alerts.md#aigenrecoveryindexrepairblocked |
...
| `readmates.aigen.tokens` | counter | `provider`, `model`, `direction` (`input`, `cache_write_input`, `cache_read_input`, `output`) | token | 내부 4채널 token 사용량. 공개 REST의 input/cachedInput/output 3필드와 구분한다. | `server/.../aigen/application/service/AiGenerationMetrics.kt` | dashboards.md#ai-session-generation | — |
...
| `readmates.aigen.grounding.repairs` | counter | `status` (`SUCCEEDED` / `FAILED`) | 건수 | 전체 문맥을 유지한 bounded single-section repair 결과. | `server/.../aigen/application/service/AiGenerationMetrics.kt` | — | — |
...
| `readmates.aigen.queue.depth` | gauge | (없음) | 건수 | Scheduler가 sampling한 authoritative processing-recovery index의 `PENDING` + `RUNNING` 수. 성공 시 유한한 실제 값(0 포함), probe unavailable이면 `NaN`이다. Gauge callback은 Redis I/O를 하지 않는다. | `server/.../aigen/application/service/AiGenerationQueueProbeService.kt`, `server/.../aigen/application/service/AiGenerationQueueDepthGaugeBinder.kt` | dashboards.md#ai-session-generation | alerts.md#aigenqueuelaghigh |
| `readmates.aigen.queue.probe.available` | gauge | (없음) | boolean | 최신 sample이 authoritative이면 1, Redis 실패·repair epoch 미완료·ceiling 초과·quarantine이면 0. | `server/.../aigen/application/service/AiGenerationQueueProbeService.kt` | dashboards.md#ai-session-generation | alerts.md#aigenqueueprobeunavailable |
| `readmates.aigen.queue.probe.last.success.timestamp.seconds` | gauge | (없음) | Unix seconds | 마지막 성공 sample 시각. 첫 성공 전 `NaN`; 실패 sample은 기존 성공 시각을 보존한다. Age는 `time() - metric`으로 계산한다. | `server/.../aigen/application/service/AiGenerationQueueProbeService.kt` | dashboards.md#ai-session-generation | alerts.md#aigenqueueprobestale |
| `readmates.aigen.queue.probe.sample.interval.seconds` | gauge | (없음) | 초 | Typed `queue-probe-fixed-delay`의 유한한 양수 초 값. Dynamic freshness 비교의 기준이다. | `server/.../aigen/application/service/AiGenerationMetrics.kt` | dashboards.md#ai-session-generation | alerts.md#aigenqueueprobestale |
| `readmates.aigen.provider.calls` | counter | `provider`, `status` | 건수 | 실제 provider coordinator call의 bounded circuit outcome. | `server/.../aigen/application/service/AiGenerationMetrics.kt` | dashboards.md#ai-session-generation | alerts.md#aigenprovidercircuitopen |
| `readmates.aigen.provider.call.latency` | timer/histogram | `provider`, `status` | 초 | permit/reservation 이후 provider call latency. Exemplar로 content-free trace를 연결한다. | `server/.../aigen/application/service/AiGenerationMetrics.kt` | dashboards.md#ai-session-generation | — |
| `readmates.aigen.provider.gate.rejections` | counter | `provider`, `reason` | 건수 | circuit/semaphore가 physical request 전에 거절한 횟수. Slot/cost를 소비하지 않는다. | `server/.../aigen/application/service/AiGenerationMetrics.kt` | dashboards.md#ai-session-generation | — |
| `readmates.aigen.provider.circuit.state.transitions` | counter | `provider`, `status` | 건수 | AI provider circuit의 bounded state transition. | `server/.../aigen/application/service/AiGenerationMetrics.kt` | dashboards.md#ai-session-generation | alerts.md#aigenprovidercircuitopen |
| `readmates.aigen.provider.cost.usd` | counter | `provider`, `basis` (`ACTUAL`, `ESTIMATED_UNKNOWN`) | USD | 실제/보수적 unknown cost 정산을 구분한다. | `server/.../aigen/application/service/AiGenerationMetrics.kt` | dashboards.md#ai-session-generation | alerts.md#aigenestimatedunknowncostgrowth |
| `readmates.aigen.provider.physical.call.cap.exhaustions` | counter | `provider` | 건수 | 최대 3회 물리 호출 cap 소진 job. | `server/.../aigen/application/service/AiGenerationMetrics.kt` | dashboards.md#ai-session-generation | alerts.md#aigenphysicalcallcapexhausted |
| `readmates.observability.otlp.export.spans` | counter | `status` (`exported`, `failed`) | span | bounded exporter가 처리한 span outcome. Product 성공/실패와 구분한다. | `server/.../shared/observability/MeteredSpanExporter.kt` | dashboards.md#ai-session-generation | alerts.md#aigenotlpexporterdrops |
| `readmates.observability.otlp.queue.dropped.spans` | counter | (없음) | span | exporter에 도달하기 전 bounded queue drop의 보수적 lower bound. | `server/.../shared/observability/OtlpSpanDeliveryTracker.kt` | dashboards.md#ai-session-generation | alerts.md#aigenotlpexporterdrops |
