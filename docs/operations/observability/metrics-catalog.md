...
| `readmates.notifications.delivery.backlog` | gauge | `status` (`pending` / `failed` / `dead` / `sending`) | 행수 | status별 `notification_deliveries` 적체. 첫 refresh 전에는 `NaN`이며, 조회 실패는 마지막 성공 snapshot을 보존한다. | `server/.../ReadmatesOperationalMetrics.kt` | dashboards.md#notification-pipeline | alerts.md#notificationdeliverybackloghigh |
