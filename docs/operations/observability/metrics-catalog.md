...
|------|------|------|------|------|----------|------|------|
| `readmates.notifications.outbox.backlog` | gauge | `status` (`pending` / `failed` / `dead` / `publishing`) | 행수 | status별 `notification_event_outbox` 적체 행 수. 마지막 성공 snapshot을 60초마다 갱신하며 조회 실패에는 거짓 0을 쓰지 않는다. | `server/.../ReadmatesOperationalMetrics.kt`, `server/.../CachedNotificationBacklogProvider.kt` | dashboards.md#notification-pipeline | alerts.md#notificationoutboxbackloghigh |
| `readmates.notifications.delivery.backlog` | gauge | `status` (`pending` / `failed` / `dead` / `sending`) | 행수 | status별 `notification_deliveries` 적체 행 수. event outbox 지표와 교환해 해석하지 않는다. | `server/.../ReadmatesOperationalMetrics.kt` | dashboards.md#notification-pipeline | alerts.md#notificationdeliverybackloghigh |
| `readmates.outbox.publish` | counter | `result` (`success` / `failure` / `dead` / `missing_payload` / `stale_lease`) | 건수 | event relay publish transition이 실제 commit된 결과. raw provider 오류나 event ID는 label에 넣지 않는다. | `server/.../NotificationRelayService.kt` | dashboards.md#notification-pipeline | slos.md#notification_dispatch_success_ratio |
