...
AI generation job은 Redis hash와 transcript/result payload key를 분리해 운영합니다. 정상 상태 전이는 다음과 같습니다.

```text
PENDING -> RUNNING -> SUCCEEDED -> COMMITTING -> COMMITTED
                     |              |
                     |              +-- commit 실패 시 SUCCEEDED로 복구
                     +-- regenerate는 SUCCEEDED에서만 result 교체
...
- `aigen:job:<jobId>` hash는 status, stage, token/cost 누적값, `llmCallCount`, model metadata를 담고 TTL까지 유지됩니다. Transcript 본문은 hash field에 넣지 않습니다.
- `aigen:job:<jobId>:transcript`와 `aigen:job:<jobId>:result`는 transient payload입니다. `COMMITTED` 또는 `CANCELLED`로 전이된 뒤에는 삭제되고, terminal hash만 TTL까지 남아 frontend polling이 최종 상태를 확인할 수 있습니다.
- `llmCallCount`는 full generation, provider retry, validation retry, regeneration 호출 전마다 증가합니다. `readmates.aigen.job.max-llm-calls-per-job`를 넘으면 provider 호출 없이 `MAX_CALLS_EXCEEDED`가 반환됩니다.
...
`COMMITTED`/`CANCELLED`인데 result/transcript key가 없는 것은 정상입니다. `PENDING`/`RUNNING`/`SUCCEEDED`에서 transcript key가 사라진 job은 stale job으로 간주되어 load 시 정리될 수 있습니다.
...
2. Audit log 조회:
   ...
   ```
3. Redis에 같은 jobId hash가 살아 있으면 (`aigen:job:<jobId>` 6h TTL) 운영자가 status/stage와 결과 JSON 존재 여부를 확인할 수 있습니다. Hash 조회는 transcript 본문을 반환하지 않습니다. `COMMITTED`/`CANCELLED` 이후에는 transient payload가 삭제되므로 `:result`가 없어도 정상입니다:
   ```bash
   redis-cli -h <host> -a <password> --no-auth-warning HGETALL "aigen:job:<jobId>"
   redis-cli -h <host> -a <password> --no-auth-warning EXISTS "aigen:job:<jobId>:result"
   redis-cli -h <host> -a <password> --no-auth-warning TTL "aigen:job:<jobId>:transcript"
...
CI는 모든 PR에서 `scripts/aigen-pii-check.sh`를 실행합니다. Fail 시 PR이 차단됩니다. 5개 invariant는 spec §11.5와 현재 Redis handoff 설계의 PII 보호 약속을 코드 레벨로 검증합니다. MySQL audit log, Kafka message, metrics, operator ticket/chat/log에는 transcript 본문이 들어가면 안 됩니다. Redis `aigen:job:<jobId>:transcript`는 worker handoff를 위한 short-lived raw transcript key로만 허용되며 TTL/delete integration test로 보호합니다.
...
1. 로컬 재현:
   ...
   ```
2. 출력의 실패 메시지는 어느 invariant가 (1-5) 깨졌는지와 file:line을 가리킵니다.
3. 다음 중 하나로 대응:
