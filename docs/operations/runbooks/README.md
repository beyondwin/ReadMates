...
- [Release bypass ledger](release-bypass-ledger.md) — solo-admin 또는 incident 대응으로 branch protection을 우회할 때 남길 공개 안전 증거와 후속 조치.
...
- [AI session generation](ai-session-generation.md) — legacy/grounded pipeline rollout, model capability, cost admission, provider key 회전, commit recovery, kill switch, alert/PII 진단 절차.
