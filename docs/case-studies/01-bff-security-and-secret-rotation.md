...
> Cloudflare Pages Functions를 단순 프록시가 아니라 보안 경계로 운영했습니다. cookie domain strip, 내부 추적 헤더 차단, client IP 정규화를 `_shared/proxy.ts` 단일 헬퍼에 응집시켰고, BFF↔Spring 신뢰는 multi-secret(primary + rotation candidates) + audit table로 무중단 회전이 가능하게 만들었습니다. 결과적으로 운영 중 secret 회전은 Spring 후보 추가 → Cloudflare primary 전환 → old secret 제거 순서의 절차로 정리됐고, 기본 `rotation-only` audit mode에서는 non-primary alias(`secondary`/`index_N`) 사용만 `bff_secret_rotation_audit` 테이블에 기록합니다.
...
### Spring 측 secret 검증 — SecretComparator + BffSecretFilter
...
`MessageDigest.isEqual`은 길이가 같으면 상수 시간 비교를 수행합니다. `SecretComparator`는 매칭이 성공한 뒤에도 모든 candidates를 끝까지 순회합니다. 이는 timing side-channel으로 어느 index의 secret이 유효한지 추론하지 못하게 하는 구현입니다. `BffSecretFilter.aliasFor`는 `firstMatchingIndex` 결과를 `primary`/`secondary`/`index_N` alias로 변환하고, 같은 comparator를 `RateLimitFilter`도 사용해 BFF 인증과 rate-limit trust boundary가 같은 비교 규칙을 따릅니다.

매칭이 성공하면 `auditAsync`가 `readmates.security.bff.audit-mode` 설정에 따라 비동기로 `bff_secret_rotation_audit`에 row를 기록합니다. 기본값 `rotation-only`는 회전 확인에 필요한 non-primary alias만 기록합니다. 모든 성공 요청을 기록해야 하는 짧은 incident window에서는 `all`, DB 압박 상황의 임시 우회에는 `off`를 사용할 수 있습니다.
...
rotation 진행 상황은 alias 분포로 확인합니다: 기본 `rotation-only` 모드에서 `secondary` 또는 `index_N`의 최근 row가 없으면 구 secret traffic이 사라진 것입니다.
...
- **audit volume trade-off**: `rotation-only` 기본값은 평상시 DB 적재량을 줄이지만 primary alias 요청은 기록하지 않습니다. 전체 요청 감사가 필요한 incident window에서는 `READMATES_SECURITY_BFF_AUDIT_MODE=all`을 명시해야 합니다.
...
- **shared fallback에서의 host 헤더**: `readmates.pages.dev`(shared fallback 호스트)가 항상 `X-Readmates-Club-Host`로 전송됩니다. 이 호스트는 `club_domains` 테이블에 없어 DB lookup이 miss됩니다. Case 03의 incident(`current-session refresh`)가 이 흐름에서 발생했고, ADR-0013 이후 서버가 `HOST_FALLBACK` miss를 unscoped auth로 처리해 같은 bug class를 막습니다.
...
- **BFF host 헤더 가시성**: ADR-0013은 BFF host 헤더를 유지하고 서버에서 source-aware하게 해석하는 방향으로 정리했습니다. 다음 개선은 shared fallback miss와 custom domain miss를 운영 로그나 low-cardinality metric으로 구분하는 것입니다.

## 관련

- ADR-0001 — Cloudflare Pages Functions BFF 채택 (`docs/development/adr/0001-cloudflare-pages-functions-bff.md`)
- ADR-0005 — BFF shared secret + multi-secret rotation (`docs/development/adr/0005-bff-shared-secret-with-rotation.md`)
- ADR-0014 — BFF secret rotation lifecycle (`docs/development/adr/0014-bff-secret-rotation-lifecycle.md`)
