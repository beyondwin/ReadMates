...
작성일: 2026-05-08
기반 문서: `docs/improvements.md`
대상 버전: `v1.5.2` 이후

## 개요

본 문서는 `docs/improvements.md`에서 도출된 7개 카테고리(아키텍처/백엔드/프런트/인프라/보안/테스트/DX) 개선사항을 **PR 단위로 실행 가능한 태스크 36개**로 분해한 실행 계획이다. 각 태스크는 독립적으로 머지 가능하도록 인터페이스 시그니처, 분할 후 파일 트리, 검증 명령까지 명시한다.
...
- **구현 방법**: `READMATES_BFF_SECRET=local-dev-secret`(`.gitleaks.toml` allowlist), MySQL/Redis URL placeholder를 한 곳에 모아 `./server/gradlew -p server bootRun`을 실행. set -euo pipefail.
