...
DB migration이 포함된 릴리즈는 같은 tag의 backend를 먼저 올리고 Spring startup Flyway와 health를 확인한 뒤에만 호환되는 Pages BFF/frontend를 배포합니다. 현재 Unreleased는 V52–V56을 additive하게 적용하며 host cursor와 mutation identity current HMAC key가 backend startup의 필수 조건입니다. 두 current key를 GitHub Secrets에 먼저 provision하고 version Variables를 확인한 다음 `sync-config(restart_api=false, dry_run=false)`를 성공시켜야 backend promotion을 시작할 수 있습니다. 실패 시 migration을 되돌리거나 이미 발행한 tag를 이동하지 않고 schema와 immutable evidence를 보존한 compatible image 또는 새 forward-fix tag로 복구합니다.
