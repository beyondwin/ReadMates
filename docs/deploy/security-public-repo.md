...
`scripts/public-release-check.sh`가 `deploy/oci/{prometheus,alertmanager,grafana}/`, `ops/prometheus/alerts/`, `ops/tempo/`에 예시값이 아닌 이메일 도메인이나 IPv4 literal이 들어오면 fail시킨다. Prometheus target은 docker network DNS(`readmates-api:8081`, `alertmanager:9093`)만 사용한다. Grafana는 운영 VM의 `127.0.0.1:3001`에만 바인딩하고 SSH tunnel로 접근하며 Tempo/OTLP는 OCI host port를 publish하지 않는다. Production AI config validator는 app의 internal `tempo:4318` endpoint, legacy selector 부재, Google paid-tier confirmation의 fail-closed sync를 함께 확인한다.
