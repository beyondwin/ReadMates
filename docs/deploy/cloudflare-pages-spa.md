...
브라우저는 `/oauth2/authorization/google`에서 로그인을 시작합니다. Pages Functions가 Spring으로 요청을 전달하고, Spring이 Google로 redirect하며, Google callback은 `READMATES_AUTH_BASE_URL`의 `/login/oauth2/code/google`로 돌아옵니다. 프런트엔드는 같은 origin의 안전한 relative `returnTo`만 OAuth start에 전달하고, 성공 후 signed `returnTo`가 검증되면 원래 클럽 path나 active registered host로 복귀합니다. 없으면 `/app` smart entry로 이동합니다.
