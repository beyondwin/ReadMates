...
`server/Dockerfile`은 로컬에서 재현 가능한 multi-stage image build용입니다. `server/Dockerfile.release`는 server image publish workflow가 `clean test bootJar`를 통과한 뒤 사용하며, scan되는 digest가 CI에서 검증된 jar와 정확히 일치하게 합니다. 두 파일의 runtime instruction은 계속 정렬되어 있어야 합니다.
