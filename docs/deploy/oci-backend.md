...
3. DB backup을 만듭니다.
4. `05-deploy-compose-stack.sh`로 image, compose file, Caddyfile, systemd unit을 배포합니다.
...
- Compose Caddy 로그는 container stdout으로 확인합니다. Legacy host Caddy rollback에서는 `/var/log/caddy/readmates.log`를 확인합니다.
