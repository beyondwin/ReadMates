...
- Grafana credential apply repair: the deploy script now resets the Grafana admin password after startup using the container's actual Grafana home/config paths so regenerated VM-local credentials are accepted when a persisted Grafana volume already exists.
