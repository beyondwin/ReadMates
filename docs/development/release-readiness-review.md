...
- Runtime apply repair: the deploy script now calls Prometheus lifecycle reload after `compose up` so an already-running Prometheus container applies changed scrape/rule config during the same deploy.
- Grafana credential apply repair: the deploy script now resets the Grafana admin password after startup so regenerated VM-local credentials are actually accepted when a persisted Grafana volume already exists.
