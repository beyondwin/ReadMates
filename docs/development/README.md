...
- Durable product·technical·design·operational decision의 canonical registry는 [ADR 인덱스](adr/README.md)입니다. [technical-decisions.md](technical-decisions.md)는 링크 호환용 파생 인덱스입니다. `Accepted` 결정은 앞으로의 판단 근거로 먼저 확인하고, `Proposed` 결정은 구현·테스트·architecture 동기화 전까지 현재 동작으로 읽지 않습니다.
