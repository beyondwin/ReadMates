...
07–17 PNG는 page composition의 시각 권위이고, code-native UI는 편집·runtime source다. **최종 실행 권위는 실제 authenticated route이며 component fixture와 tracked CT snapshot은 보조 회귀 근거일 뿐 최종 승인 receipt를 만들지 않는다.** PNG를 runtime 배경으로 쓰지 않고 snapshot 갱신만으로 합격하지 않는다. token, shared CSS/component, fixture 변경은 영향 reference 증거를 무효화한다. 전체 capture에 적용하던 broad font-raster 예외는 제거했고 gate는 18/18 `0.02` fail-closed다. 현재 시각 권위 기록은 `docs/reports/2026-09-04-admin-host-actual-route-visual-authority-acceptance.md`이며 composition·geometry·typography·first viewport·interaction은 18/18 통과, strict pixel은 18/18 `not_passed_0.02`다. 사람 30초 gate와 보조기술·원격 CI가 남아 픽셀 수락 완료가 아니다. ADR-0053은 `Proposed`다.

Responsive·interaction 근거 중 2026-09-04 실행에서 **통과한** 것은 [모임 생애주기 CT](../../../front/features/host/ui/meeting-workspace/host-lifecycle-responsive.ct.tsx), [호스트 shell CT](../../../front/features/host/ui/shell/host-shell.ct.tsx), [route continuity E2E](../../../front/tests/e2e/host-lifecycle-route-continuity.spec.ts)다. [authority-loss E2E](../../../front/tests/e2e/host-authority-loss.spec.ts)는 이번 수렴에서 **실행하지 않았으므로** 통과 근거로 읽지 않는다.

> **Stale 근거 주의.** [workbox E2E](../../../front/tests/e2e/host-workbox-stage4.spec.ts), [운영실 CT](../../../front/features/host/ui/operating-room/host-operating-room-responsive.ct.tsx), [승인 원장 CT](../../../front/features/host/ui/approved-host-ledgers.ct.tsx)는 2026-09-04 기준 **실패 중**이며 현재 계약의 근거로 인용하지 않는다. 이 spec들은 작업함 desktop 4건·mobile 3건 cap 이전의 기대치를 그대로 담고 있다(예: `host-workbox-stage4.spec.ts`가 작업함 listitem 40개를 기대하지만 실제는 4개다). 갱신 전까지는 회귀 근거가 아니라 미해결 잔여이며, 전체 목록과 exit code는 `docs/reports/2026-09-04-admin-host-actual-route-visual-authority-acceptance.md` §잔여 위험과 릴리스 경계에 있다. **CT snapshot을 갱신해 이 차이를 흡수하지 않는다.**

승인 PNG 비교는 `front/tests/e2e/support/approved-mockup-manifest.ts`와 실제 authenticated route를 candidate로 쓰는 `pnpm --dir front test:e2e:approved-routes:docker`가 담당한다. `pnpm --dir front test:ct:docker`는 보조 component 회귀 suite이며 승인 receipt를 만들지 않는다. 접근성 근거는 [bounded DOM/ARIA helper](../../../front/tests/e2e/support/visual-authority-contract.ts)를 사용한 코드 기반 검사이며 axe/axe-core나 수동 VoiceOver·NVDA 전체 검증으로 확대 해석하지 않는다.
