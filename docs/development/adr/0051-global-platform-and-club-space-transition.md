...
- 실제 `GlobalSpaceTransitionController` wrapper와 `useTransitionSafetyOwner`를 함께 쓰는 host/platform 통합 검증은 `beginPending → accepted settlement → async publication await → authority loss → resume` 순서에서 원래 request count 불변, replay 0회, obsolete publication, `authority-lost` reconciliation, UI/cache/refetch/receipt callback/copy/navigation/return-target/session-storage publication 0회를 고정한다. Normal unmount의 same-identity receipt lookup·`finally` clear는 이 authority-loss zero-replay 경로와 별도 coordinator test로 검증한다.
