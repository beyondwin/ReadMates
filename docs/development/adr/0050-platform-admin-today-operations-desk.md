...
- `front/features/platform-admin/model/admin-copy.ts`, `front/features/platform-admin/model/admin-status-language.ts`와 UI 경계 검증이 raw enum/ID와 특정 club content를 1급 정보로 올리지 않도록 고정한다. Primary-language source analyzer는 126/127/128 lexical alias 경계에서 depth exhaustion을 명시적으로 fail close하고, injected disclosure contract와 입력 filename 충돌로 분석 대상을 잃지 않는다. Case action의 primary copy는 lifecycle 언어를 사용하되 기술 정보는 원래 action 값을 보존한다.
