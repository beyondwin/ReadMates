...
- OCI OCID(Oracle Cloud resource identifier): `<oci-resource-id>` 형식. 운영 인프라 자원 식별자가 노출되면 대상이 특정된다.
...
공개 manifest는 `copy_manifest()`에서 명시적으로 enumerate 되고, manifest 외 경로가 staging 결과에 남아 있으면 `verify_candidate()`가 거부한다. 승인/거부 판단은 `is_approved_manifest_path()`와 `is_forbidden_candidate_path()`로 모듈화되어 있다. 주요 포함 경로:
- `front/`, `design/`, `server/`, `deploy/oci/` (전체, build output와 `deploy-state`/`*.state` 제외), `docs/deploy/`, `docs/development/`
- `docs/operations/` 중 `README.md`와 `runbooks/`만 포함 — 나머지 운영 문서는 의도적으로 제외
- `docs/superpowers/` 하위 문서는 private historical 작업 기록으로 간주해 공개 후보에서 제외
- 루트 필수 파일: `.github/workflows/{ci,deploy-front,deploy-server}.yml`, `.gitignore`, `.env.example`, `.node-version`, `README.md`, `PRODUCT.md`, `compose.yml`, `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`
- `scripts/` 중 공개 운영 보조 스크립트: `build-public-release-candidate.sh`, `pre-push-check.sh`, `public-release-check.sh`, `server-ci-check.sh`, `smoke-production-integrations.sh`, `verify-public-release-fixtures.sh`, `README.md`

금지 경로 패턴은 `is_forbidden_candidate_path()`에 정의된다 (대표 항목, 전체 목록은 스크립트 참조):
...
경로 패턴 기반 금지:
- 모든 `node_modules`, `design/standalone`, `design/*/dist`, `front/{dist,output,coverage,test-results,playwright-report,.nyc_output}`, `server/{build,.gradle,.kotlin}` — 로컬·빌드·테스트 산출물
- `.gstack/`, `.superpowers/`, `.idea/`, `.playwright-cli/` — 내부 도구
- `.wrangler/`, `.cloudflare/`, `.vercel/`, `.terraform/`, `.pulumi/` — cloud provider 로컬 상태
- `private/`, `recode/`, `.tmp/`, `output/`, `deploy/oci/.deploy-state` — 내부 전용/배포 상태
- `.claude/`, `.cursor/`, `.windsurf/`, `.orchestrator/`, `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `.impeccable.md`, `*.local.md` — AI 어시스턴트/IDE 컨피그
- `CHANGELOG.md` — 내부 릴리즈 노트 (공개 manifest 외)
- 스크린샷류 (`screenshot`, `screenshots`, `*screenshot*`, `*screen-shot*`)
- 추가 키 자료: `id_rsa*`, `id_ed25519*`, `id_ecdsa*`, `id_dsa*`

특수 예외: `.env.example`은 명시적으로 허용된다.
...
regex = '''<oci-resource-id-regex>'''
keywords = ["<oci-resource-id-prefix>"]
...
1단계에서 생성한 clean manifest만 scanner가 검사한다. `node_modules`, `server/build`, `design/standalone`, `design/*/dist`, `.gstack/` 같은 generated·내부 경로가 scanner 대상에서 미리 제외된다. 반면 frontend 빌드와 CI에 필요한 `design/system`, `design/docs`, 루트 pnpm 계약은 포함해 후보 자체의 재현 가능성을 유지한다. scanner가 더 빠르고 false positive가 줄어든다.
...
- `scripts/pre-push-check.sh`로 푸시 전 local 단계에서도 release-sensitive 경로 변경 시 동일 scanner를 자동 실행해 CI 도달 전에 조기 차단된다.
...
- fixture에 OCI OCID 형식 문자열(`<oci-resource-id-example>` — 실제 fixture는 스크립트 런타임에 탐지 대상 패턴을 조립) 삽입 → `verify-public-release-fixtures.sh` 실행 → `readmates-oci-ocid` finding 보고 확인
...
- `scripts/pre-push-check.sh`를 `.git/hooks/pre-push`로 자동 설치하는 onboarding 스크립트 검토 (현재는 옵트인 설치).
