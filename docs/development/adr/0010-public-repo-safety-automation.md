...
- `front/`, `server/`, `deploy/oci/` (전체, `deploy-state`/`*.state` 제외), `docs/deploy/`, `docs/development/`
...
- 루트 필수 파일: `.github/workflows/{ci,deploy-front,deploy-server}.yml`, `.gitignore`, `.env.example`, `README.md`, `compose.yml`
...
경로 패턴 기반 금지:
- `node_modules`, `front/{dist,output,coverage,test-results,playwright-report,.nyc_output}`, `server/{build,.gradle,.kotlin}` — 빌드/테스트 산출물
- `design/`, `.gstack/`, `.superpowers/`, `.idea/`, `.playwright-cli/` — 내부 도구
