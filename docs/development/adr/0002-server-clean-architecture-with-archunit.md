...
ArchUnit(`server/build.gradle.kts:45`: `testImplementation("com.tngtech.archunit:archunit-junit5:1.3.0")`)은 JVM 바이트코드를 분석해 패키지 간 의존 방향을 테스트로 표현한다. 규칙 위반 시 `./server/gradlew test`가 실패한다.
