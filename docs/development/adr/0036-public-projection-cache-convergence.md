...
- 같은 날 V55 emergency server substrate가 origin deny와 generation 회전, immutable redacted admin receipt, 기존 V54 convergence link/work를 한 transaction에 연결했다. Club list/stats/detail과 PUBLISHED guest reader는 exact generation의 readable/non-denied marker를 요구하고, CLOSED archive는 markerless compatibility와 marker-present deny를 구분한다. Deterministic actual-BFF harness가 60초 뒤 revoked club list를 재조회하는 경계를 검증한다. Production confirm은 protected Step 8 evidence verifier가 없어서 계속 fail closed이며, operator UI·active runbook·실제 CDN runtime evidence도 후속 작업이므로 상태는 `Proposed`다.
