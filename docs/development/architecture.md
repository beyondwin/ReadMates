...
| 호스트 앱 | `/clubs/:slug/app/host`, `/clubs/:slug/app/host/sessions`, `/clubs/:slug/app/host/people`, `/clubs/:slug/app/host/records`, `/clubs/:slug/app/host/settings`, `/clubs/:slug/app/host/notifications`, `/clubs/:slug/app/host/sessions/new`, `/clubs/:slug/app/host/sessions/:sessionId`, 등록된 club host의 `/app/host/**` | 현재 클럽의 호스트 | 1차 내비게이션은 운영실·일정과 모임·사람·기록 네 영역입니다(ADR-0048 Proposed). 운영실은 서버가 고른 현재 모임의 준비실→현장→마감실, 다음 행동 하나, 준비 현황과 immutable-snapshot 작업함을 조합합니다. `/members`, `/invitations`, `/operations`는 각각 `/people`, `/settings#invitations`, 운영실로 query/hash와 검증된 return state를 보존해 replace하고 `/records`는 canonical 기록 원장으로 유지합니다. 일정 revision 확인은 접속·응답·출석·알림 전달과 독립이고, 일정 미열람 검토는 editable preview 뒤 명시적으로 발송합니다. 사람 상세, named invitation link, club settings/co-host history와 local-safe 운영 종료 preview/confirm을 제공합니다. |
...
호스트 primary route는 운영실 `/app/host`, 일정과 모임 `/app/host/sessions`, 사람 `/app/host/people`, 기록 `/app/host/records`이며 scoped route는 앞에 `/clubs/:slug`를 붙입니다. `/app/host/settings`, `/notifications`, `/sessions/new`는 utility/action입니다. `/members`, `/invitations`, `/operations`는 각각 `/people`, `/settings#invitations`, 운영실로 query/hash와 검증된 same-club return state를 보존해 replace합니다. `/records`는 redirect가 아니라 canonical 기록 원장입니다. Route inventory와 continuity는 `front/src/app/route-continuity.ts`, `front/src/app/routes/host.test.tsx`, `front/tests/e2e/host-lifecycle-route-continuity.spec.ts`가 소유합니다.
