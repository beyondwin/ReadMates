...
Admin `design/mockups/2026-08-30-admin-operations-redesign/` `01`–`07`과 Host `docs/development/host-redesign-mockups/` `07`–`17` 승인 PNG는 해당 화면의 시각 권위입니다. code-native UI는 편집·runtime source이고 tracked snapshot은 보조 회귀 cache입니다. token·shared CSS·fixture 변경은 영향 reference 증거를 무효화합니다. 픽셀 근접 합격 기준(ADR-0053)은 독립 시각 검토와 사람 30초 gate가 남아 `Proposed`입니다. 현재 구현 composition은 [`front/DESIGN.md`](../../front/DESIGN.md)를 따릅니다.
