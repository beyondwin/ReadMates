...
V55의 admin idempotency scope는 `(platformAdminUserId, EMERGENCY_PUBLIC_TAKEDOWN, clubId, publicationId, key)`이며, reason 원문은 기존 secret-keyed request identity primitive로 HMAC한 뒤 versioned canonical request HMAC에만 포함합니다. Duplicate claim은 locking current read로 completed idempotency row와 receipt를 재조회하므로 같은 key/request 경쟁은 immutable winning receipt에 수렴하고 다른 request는 conflict입니다. 새 confirm 한 번은 application-service transaction에서 origin을 deny하고 generation을 정확히 한 번 회전한 뒤 redacted admin receipt, `platform_audit_events.EMERGENCY_PUBLIC_TAKEDOWN_CONFIRMED`, V54 convergence link/work를 한 번씩 기록합니다. BFF eviction은 `NOT_STARTED`, CDN은 `QUEUED`, browser는 cache-policy revalidation로 구분하며 provider attempt는 transaction 밖 Task 2 convergence processor가 같은 `convergenceId` 아래 append-only로 추가합니다. Preview/idempotency row는 bounded retention 대상이고 purge pass는 admin idempotency, preview, host idempotency에 namespace-fair budget을 배분하면서 전체 limit과 actual deleted count를 유지합니다. Content FK가 없는 redacted admin/convergence evidence는 session/publication hard delete 뒤에도 남습니다.
