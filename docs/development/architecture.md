...
| 플랫폼 관리 | `/admin`, `/admin/today`, `/admin/health`, `/admin/notifications`, `/admin/clubs`, `/admin/clubs/:clubId`, `/admin/support`, `/admin/ai-ops`, `/admin/public-takedown`, `/admin/audit`, `/admin/analytics` | platform admin | `/admin/**`의 page-level composition은 Editorial Operations Ledger입니다. 1차 내비는 오늘 · 클럽 · 파이프라인 · 원장 네 축이고, 긴급 공개 회수는 사이드 하단 비상 레인입니다. Admin shell은 navigation·capability·onboarding·authority-loss만 소유하고 domain data mega-store가 아닙니다. URL·filter·return/scroll/focus는 각 route가 소유하고, mutation은 해당 domain route의 exact capability 또는 Today `allowedActions`와 command 안전 등급이 소유합니다. `/admin/today`의 내구 운영 케이스 ledger에서 클럽 공개 readiness·domain·첫 호스트, 알림 실패, AI job 이상, 회차 마감 위험을 우선순위 queue와 inspector로 확인하고 acknowledge·snooze·엄격한 resolve 검증을 수행합니다. 그 밖에 클럽 생성, 클럽 목록 확인, 공개/비공개 상태 관리, 공개 소개 정보 관리, 등록형 domain alias 요청과 상태 확인, 첫 호스트 온보딩 상태 확인, 운영 health와 알림 outbox/delivery 상태 확인, 클럽 운영 readiness 집계, 제한된 support access grant 관리, AI job 운영 조회와 강제 취소, 긴급 공개 회수, 통합 감사 ledger 조회를 수행합니다. `/admin/analytics`는 활성 멤버, 세션 완료율, RSVP 응답률, AI 비용/세션, 알림 도달률을 7/30/90일 window와 series/benchmark로 보여주는 aggregate-only 운영 분석 표면입니다. 세션/멤버/알림 발송 같은 클럽 내부 운영은 기본적으로 호스트 앱 책임이고, platform admin 표면은 aggregate/read-only 진단과 감사 가능한 복구 작업만 다룹니다. |
...
OWNER와 OPERATOR만 case lifecycle을 변경하고 SUPPORT는 safe projection만 읽습니다. Resolve는 exact source identity가 사라졌음을 authoritative하게 재검증해야 하며 active 또는 확인 불가 signal은 fail closed합니다. Case API는 safe summary code, aggregate impact, source freshness, canonical detail link만 노출하고 raw email·recipient·transcript·provider error·생성 JSON·private member content를 투영하지 않습니다. Frontend `/admin/**` page-level 권위는 Editorial Operations Ledger입니다. Admin shell은 navigation, capability query, onboarding, authority-loss purge만 소유하고 domain data를 선행 로드하는 mega-store가 아닙니다. URL·filter·return/scroll/focus는 각 route가 소유합니다. Today lifecycle은 서버 `allowedActions`로 L1을 실행하고, 알림 replay·분석 export·긴급 회수·클럽/지원 command는 해당 domain route가 exact capability와 L1/L2/L3 문법을 조합합니다. `AdminPageContext`·`AdminWorkViewBar`·`AdminEvidenceLedger`·`AdminStatePanel`은 공통 presentation이고, `AdminCaseDocket`·`AdminSafeActionDock`·`AdminReceiptTimeline`은 선택 대상·command·receipt가 있을 때만 붙습니다. 범용 execute endpoint는 없습니다.
