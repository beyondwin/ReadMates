...
`ServerArchitectureInventoryTest`는 승인된 39개 boundary identity를 current 23개와 retired 16개로 정확히 분할하고, application feature dependency edge를 최대 41개로 고정합니다. 두 ledger는 승인된 목표 architecture가 아니라 제거 가능한 debt inventory입니다. 이 application-port 작업은 일부 non-auth/non-session-record identity만 retired로 옮겼을 뿐, actor/auth/club web helper, sessionclosing/sessionimport/sessionrecord, feature-cycle 및 large-class debt는 이후 Phase 2 계획에 남아 있습니다. 따라서 boundary debt는 의도적으로 23개이며 Phase 2 완료나 baseline zero를 뜻하지 않습니다. `com.readmates.shared.adapter.in.web`은 그때까지 공통 web-error contract로 유지합니다.
