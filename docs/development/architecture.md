...
| 호스트 앱 | `/clubs/:slug/app/host`, `/clubs/:slug/app/host/sessions`, `/clubs/:slug/app/host/people`, `/clubs/:slug/app/host/records`, `/clubs/:slug/app/host/settings`, `/clubs/:slug/app/host/notifications`, `/clubs/:slug/app/host/sessions/new`, `/clubs/:slug/app/host/sessions/:sessionId`, 등록된 club host의 `/app/host/**` | 현재 클럽의 호스트 | 1차 내비게이션은 운영실·일정과 모임·사람·기록 네 영역입니다(ADR-0048 Accepted). 운영실은 서버가 고른 현재 모임의 준비실→현장→마감실, 다음 행동 하나, 준비 현황과 immutable-snapshot 작업함을 조합합니다. `/members`와 `/operations`는 query·허용된 incoming fragment·검증된 return state를 보존해 각각 `/people`과 운영실로 replace합니다. `/invitations`는 query와 검증된 return state를 보존하되 incoming fragment를 canonical `#invitations`로 교체해 `/settings#invitations`로 replace합니다. `/records`는 canonical 기록 원장입니다. 일정 revision 확인은 접속·응답·출석·알림 전달과 독립이고, 일정 미열람 검토는 editable preview 뒤 명시적으로 발송합니다. 사람 상세, named invitation link, club settings/co-host history와 local-safe 운영 종료 preview/confirm을 제공합니다. |
| 플랫폼 관리 | `/admin`, `/admin/today`, `/admin/health`, `/admin/notifications`, `/admin/clubs`, `/admin/clubs/:clubId`, `/admin/support`, `/admin/ai-ops`, `/admin/public-takedown`, `/admin/audit`, `/admin/analytics` | platform admin | `/admin/**`의 page-level composition은 Editorial Operations Ledger입니다. 1차 내비는 `오늘 할 일`·`클럽 관리`·`서비스 상태`·`처리 기록` 네 축이고, 긴급 공개 회수는 사이드 하단 비상 레인입니다 (`front/features/platform-admin/model/admin-route-catalog.ts`). Shell controller는 capability·alarm·platform authority loss와 account action을 조정하고, onboarding은 `/admin/clubs?onboarding=1`의 clubs route controller가 소유합니다. URL·filter·selection·return/scroll/focus와 mutation settlement는 각 domain route/controller가 소유합니다. `/admin/today`는 서버 `allowedActions`에 있는 `확인함`·`잠시 미룸`·`처리함`만 실행하고, 본문 폭 960px 이상에서는 38:62 queue/docket, 그 아래에서는 URL-addressable list/detail로 동작합니다. 클럽 관리, 서비스 상태, 알림·AI detail, 지원 접근, 처리 기록·분석 부록과 긴급 공개 회수는 같은 evidence/action/result 문법을 사용하되 기존 exact capability와 L1/L2/L3 safe-command를 유지합니다. 세션/멤버/알림 발송 같은 클럽 내부 운영은 기본적으로 호스트 앱 책임이고, platform admin 표면은 aggregate/read-only 진단과 감사 가능한 복구 작업만 다룹니다. |

Admin `design/mockups/2026-08-30-admin-operations-redesign/` `01`–`07`과 Host `docs/development/host-redesign-mockups/` `07`–`17` 승인 PNG는 해당 화면의 시각 권위입니다. code-native UI는 편집·runtime source이고 tracked snapshot은 보조 회귀 cache입니다. token·shared CSS·fixture 변경은 영향 reference 증거를 무효화합니다. 픽셀 근접 합격 기준(ADR-0053)은 독립 시각 검토와 사람 30초 gate가 남아 `Proposed`입니다. 현재 구현 composition은 [`front/DESIGN.md`](../../front/DESIGN.md)를 따릅니다. [ADR-0055](adr/0055-shared-icon-and-shell-composition.md)는 공유 paper/ink primitive와 `ReadmatesIcon`·`ReadmatesIconBadge` 사용, CSS data-URI 아이콘·`.admin-shell:has(` override 금지를 고정합니다. 후속 코드 네이티브 시각 계약은 [ADR-0054](adr/0054-code-native-design-contract.md) `Proposed`이며 현재 PNG 게이트를 대체하지 않습니다.
...
- `features/<name>/queries`는 TanStack Query key, `queryOptions`, mutation execution과 query-owned publication/invalidation 함수를 담당합니다. 전환 안전 등록이 필요한 write의 mutation hook은 transport 관측만 반환하며 자동 `onSuccess`/`onError` publication을 하지 않습니다. 등록한 route/controller가 `settle(...) === "accepted"`를 확인한 뒤에만 query-owned publisher를 호출합니다. Query module은 UI와 route module을 import하지 않습니다.
...
- 모든 깊이의 `features/**/ui/**`는 props와 callback으로만 렌더링하며 `fetch`, `shared/api`, feature API, feature queries, route/app/page module 또는 router package를 직접 import하지 않습니다. UI가 owner가 주입한 callback을 호출하는 것은 presentation composition이지 mutation 실행 소유권 예외가 아닙니다.
...
이 경계는 `front/tests/unit/frontend-boundaries.test.ts`와 `front/src/app/space-transition-producer-inventory.test.ts`에서 강제합니다 (ADR-0003, ADR-0051). 전자는 shared-to-app/page/feature import, feature 간 직접 import, feature `model/queries/route/ui` layer import, nested UI의 query/API/router/direct-fetch 금지, 제거된 `shared/api/readmates` compatibility import를 확인합니다. 후자는 mounted import graph와 repository-wide exported write를 함께 분석해 모든 write producer를 등록 owner, 수정 factory, 검증된 leaf 또는 도달 불가 export로 분류합니다.
...
인증 응답은 기존 `joinedClubs`, `platformAdmin`, `recommendedAppEntryUrl`을 유지하면서 additive `availableSpaces` v1을 제공합니다 (`server/src/main/kotlin/com/readmates/auth/application/model/AuthAccessProjection.kt`, `server/src/main/kotlin/com/readmates/auth/application/service/ResolveAuthAccessProjectionService.kt`). 전역 kind는 `PLATFORM`, `CLUBS` 순서이고, 읽을 수 있는 `VIEWER|ACTIVE|SUSPENDED` membership은 `MEMBER`, 그중 `ACTIVE HOST`만 `HOST` perspective를 추가합니다. 이 projection은 열 수 있는 제품 공간만 정하며 실제 command authority는 목적 route의 capability/guard/`allowedActions`가 다시 확인합니다. Frontend는 unknown version이나 상충하는 club identity를 빈 projection으로 fail close하고, `availableSpaces`가 없는 혼합 배포 응답에서만 legacy field로 기존보다 넓지 않은 fallback을 만듭니다 (`front/shared/auth/available-spaces.ts`).

App composition root의 `GlobalSpaceTransitionController`와 공용 `GlobalSpaceSwitcher`는 `플랫폼 운영`과 `내 클럽`의 두 단계 선택을 두 shell에 주입합니다 (`front/src/app/global-space-transition-controller.tsx`, `front/shared/ui/global-space-switcher.tsx`). Return target은 space/club/perspective별 `pathname + search + hash + focusId + scrollTop`만 versioned session storage에 보존하고, route-family allowlist와 최신 projection·authoritative entity read를 통과한 값만 복원합니다 (`front/src/app/global-space-continuity.ts`). Auth object, command body, receipt payload와 민감 content는 저장하지 않습니다.

모든 mounted write owner는 `clean|dirty|pending|unknown-outcome` 전환 coordinator에 등록됩니다 (`front/src/app/global-space-transition.ts`, `front/src/app/space-transition-producer-inventory.ts`). Query/mutation factory는 response를 관측할 뿐 cache·UI·receipt callback·copy·navigation·return target을 자동 발행하지 않고, 현재 generation의 등록 owner가 settlement를 `accepted`로 확인한 뒤 query-owned publisher를 호출합니다. Normal unmount는 receipt capsule을 retired registry에 남겨 같은 identity의 detached reconciliation만 허용하고, authority loss는 active/retired capsule과 민감 cache/draft를 먼저 invalidate/clear한 뒤 generation을 올립니다. 이전 generation의 late response와 authority-loss 뒤 reconciliation은 command를 replay하거나 어떤 publication surface도 갱신하지 않으며 representative safe route로 replace합니다.

Platform admin today command center는 `admin.operations` workflow slice가 제공하는 `/api/admin/operations/cases` 목록·상세와 acknowledge/snooze/resolve lifecycle API만 소비합니다. 이 slice는 클럽 readiness·domain·첫 호스트, 알림 실패·backlog, AI failed/stale job, 회차 마감 위험의 네 source를 allowlist된 signal로 투영하고 Flyway V47의 `admin_operation_cases`, immutable `admin_operation_case_events`, `admin_operation_source_status` ledger를 소유합니다. Case는 `OPEN`, `ACKNOWLEDGED`, `SNOOZED`, `RESOLVED`로 전이하며 동일 source identity가 다시 나타나면 같은 case가 재개방됩니다. Source 조회 실패는 `UNAVAILABLE`, 정책상 AI 비활성은 `DISABLED`로 구분하고 어느 쪽도 가짜 incident나 해결 이력을 만들지 않습니다.

OWNER와 OPERATOR만 case lifecycle을 변경하고 SUPPORT는 safe projection만 읽습니다. Resolve는 exact source identity가 사라졌음을 authoritative하게 재검증해야 하며 active 또는 확인 불가 signal은 fail closed합니다. Case API는 safe summary code, aggregate impact, source freshness, canonical detail link만 노출하고 raw email·recipient·transcript·provider error·생성 JSON·private member content를 투영하지 않습니다. Frontend `/admin/**` page-level 권위는 Editorial Operations Ledger입니다. `AdminShellController`는 navigation composition, capability query, alarm, account action과 authority-loss를 소유하고 domain data를 선행 로드하는 mega-store가 아닙니다. Onboarding mutation과 dirty/pending 상태는 `AdminOnboardingController`가 clubs route 아래에서 소유합니다. URL·filter·selection·return/scroll/focus는 각 route가 소유합니다. Today lifecycle은 서버 `allowedActions`로 L1을 실행하고, 알림 replay·분석 export·긴급 회수·클럽/지원 command는 해당 domain route가 exact capability와 L1/L2/L3 문법을 조합합니다. `AdminPageContext`·`AdminWorkViewBar`·`AdminEvidenceLedger`·`AdminStatePanel`은 공통 presentation이고, `AdminCaseDocket`·`AdminSafeActionDock`·`AdminReceiptTimeline`은 선택 대상·command·receipt가 있을 때만 붙습니다. 범용 execute endpoint는 없습니다.

Platform admin club operations는 `/api/admin/clubs/{clubId}/operations`에서 현재 클럽의 readiness, lifecycle, host/member/session counts, notification health, AI usage summary를 aggregate-only read model로 반환합니다. Support workbench는 body-only·`no-store` `/api/admin/support/search`, cursor 기반 `/api/admin/support/grants`, create/revoke `preview|confirm` route를 사용합니다. V60 support command는 OWNER capability, active platform-admin grantee, eligible club, active-slot uniqueness, 24시간 이내 만료와 allowlist `reasonCategory`를 검증하고, optional note는 request HMAC에만 결속해 DB·receipt·audit·DTO에 원문이나 별도 digest로 남기지 않습니다. Legacy free-text reason은 `LEGACY_UNCLASSIFIED + notePresent`로 즉시 redaction되며 legacy create/revoke writer는 `410`으로 fail closed합니다. Active grant의 `HOST_SUPPORT_READ`는 membership row를 만들지 않는 request-local authority synthesis만 유지합니다.

Platform admin service workbench의 notification replay와 AI force-cancel/retry는 V59 typed receipt parent와 effect target, append-only convergence attempt를 사용합니다. Origin transaction은 receipt·audit·convergence를 함께 commit하고 provider I/O는 lease/CAS transaction 밖에서 실행하며, ambiguous·retryable·permanent outcome을 성공으로 축약하지 않습니다. `/api/admin/audit/events`는 public-safe source union을 snapshot/filter/source-set에 결속한 purpose-separated V57 signed cursor로 page하고, 민감 target 검색은 `no-store` body-only `/api/admin/audit/events/search`로 제한합니다. `/api/admin/analytics/overview`는 서버가 계산한 KPI label·definition·delta·availability와 모든 series/benchmark를 반환하고, `EXPORT_ANALYTICS` capability가 있는 actor만 `/api/admin/analytics/export.csv`의 UTF-8·`no-store` CSV를 내려받습니다. CSV는 metadata, KPI, 모든 series bucket과 club benchmark를 포함하고 spreadsheet formula prefix를 escape합니다.
...
현재 full port/outbound adapter chain을 따르는 범위는 `publication`, `archive`, `feedback`, `session`, `sessionclosing`, `note`, `auth`의 운영 API surface, `notification` 운영 slice, `club`의 platform-admin 운영 slice, `admin.audit`의 platform-admin 감사 read slice, `admin.health`의 ops read slice, `admin.operations`, `admin.takedown`, `aigen`의 workflow slice입니다. Disabled password/password-reset/dev-invitation accept endpoint는 `410 Gone` stub으로 남습니다. Auth의 OAuth filter, success handler, cookie/session 보안 구성은 `auth.infrastructure.security`와 `auth.adapter.in.security`에 따로 둡니다.
...
Auth는 trusted club slug/host header의 HTTP 추출과 우선순위를 `auth.adapter.in.security`에서 소유하고 club의 resolve input port를 호출합니다. 같은 auth feature의 web/security inbound package는 이 helper를 공유할 수 있지만 다른 feature의 inbound helper를 import할 수 없습니다. Auth servlet-security filter와 OAuth success handler는 concrete service가 아니라 auth input port와 application model을 주입받습니다. Membership/access policy는 club이 소유하고, application feature 방향은 `auth -> club`만 유지합니다. Club은 invitation-token output port를 소유하고 auth가 구현하므로 역방향 `club -> auth`는 없습니다.
...
Platform admin 긴급 공개 회수는 `/api/admin/public-takedowns/preview|confirm`의 별도 `admin.takedown` workflow slice가 소유합니다. Active platform-admin 조회를 통과한 actor 중 `PlatformCapability.EMERGENCY_PUBLIC_TAKEDOWN`이 있는 OWNER/OPERATOR policy mapping만 preview를 만들 수 있고 application service는 role enum이 아니라 `actor.can(...)`으로 권한을 검사합니다. Preview는 actor role snapshot, club/session/publication UUID, generation, origin/BFF/CDN/browser 표면, 10분 TTL과 이미 저장·표시·offline인 사본을 원격 삭제할 수 없다는 경계를 고정합니다. Confirm은 동일 actor/role, TTL, current generation/surfaces, bounded reason/key와 `PRIVATE_DATA|LEGAL_REQUEST|SECURITY_INCIDENT|PUBLIC_SAFETY` category를 다시 확인합니다.

V55의 admin idempotency scope는 `(platformAdminUserId, EMERGENCY_PUBLIC_TAKEDOWN, clubId, publicationId, key)`이며, reason 원문은 기존 secret-keyed request identity primitive로 HMAC한 뒤 versioned canonical request HMAC에만 포함합니다. Duplicate claim은 locking current read로 completed idempotency row와 receipt를 재조회하므로 같은 key/request 경쟁은 immutable winning receipt에 수렴하고 다른 request는 conflict입니다. 새 confirm 한 번은 application-service transaction에서 `public_projection_generations.emergency_denied=true`, `origin_readable=false`를 함께 저장하고 generation을 정확히 한 번 회전한 뒤 redacted admin receipt, `platform_audit_events.EMERGENCY_PUBLIC_TAKEDOWN_CONFIRMED`, V54 convergence link/work를 한 번씩 기록합니다. 일반 host/session-record writer는 이 durable emergency deny를 해제하지 않으며 저장된 effective `origin_readable`을 convergence receipt에 사용합니다. PUBLISHED public reader는 exact generation의 `origin_readable=true`와 `emergency_denied=false`를 요구합니다. CLOSED guest archive는 legacy/never-published 호환을 위해 public publication marker가 없으면 읽을 수 있지만, marker가 있으면 exact generation row와 `emergency_denied=false`가 모두 필요합니다. Publication만 있고 generation이 없거나 emergency deny가 설정된 CLOSED row는 fail closed합니다. BFF eviction은 `NOT_STARTED`, CDN은 `QUEUED`, browser는 cache-policy revalidation로 구분하며 provider attempt는 transaction 밖 Task 2 convergence processor가 같은 `convergenceId` 아래 append-only로 추가합니다. Preview/idempotency row는 bounded retention 대상이고 purge batch가 3 이상이면 admin idempotency, preview, host idempotency가 pass마다 최소 한 건의 독립 budget을 받으며 전체 limit과 actual deleted count를 유지합니다. Production purge batch는 startup에서 `3..500`으로 검증하고 `0`은 adapter no-op입니다. Content FK가 없는 redacted admin/convergence evidence는 session/publication hard delete 뒤에도 남습니다.

V57의 shared admin-command claim substrate는 raw idempotency key와 canonical request를 persistence 경계 밖에 두고 `(platformAdminUserId, commandType, targetType, targetId)` scope에 key-version별 32-byte HMAC alias를 연결합니다. Claim, domain effect, immutable receipt, completion pointer는 domain application service가 소유한 한 transaction에서 처리하며 committed `IN_PROGRESS`는 takeover 없이 fail closed합니다. 완료 CAS는 receipt pointer와 completion-anchored retention을 저장합니다. Maintenance transaction은 digest-key-state를 global version order로 먼저 잠근 뒤 만료된 `COMPLETED`만 bounded purge하고 alias cascade 외의 domain receipt는 삭제하지 않습니다. `writePreviousAlias=true` overlap에서는 previous key의 `unreferenced_since`를 무효화하며, old writer drain 뒤 flag가 `false`가 된 다음 post-delete locked zero-alias 상태에서만 새 최소 24시간 retirement buffer를 시작·유지합니다. Fresh reservation은 전체 lookup key-state slot을 sorted reference/no-op upsert로 준비하고, existing replay/backfill과 duplicate loser는 missing slot까지 sorted no-op upsert한 뒤 claim/alias를 잠급니다. Actual alias reference update는 reservation savepoint 안에 남으므로 claim, replay, retirement, purge의 alias 접근이 같은 key-state-before-alias 순서로 serialize되고 missing-row gap prelock deadlock도 피합니다. 별도 post-Flyway startup validator는 state와 alias snapshot을 한 transaction에서 잠그고 missing/unknown/unconfigured referenced version, blank configured key, pending retirement, query failure를 readiness 전에 거부합니다. Micrometer tags는 allowlisted command type과 finite claim/outcome만 사용합니다.

Confirm activation의 production adapter는 조건 없이 fail closed합니다. 현재 repository에는 application property, environment variable 또는 tracked evidence로 이를 여는 경로가 없습니다. 별도 승인된 Task 1 Step 8 protected cache-safety attestation과 active runbook이 구현되기 전까지 preview response는 `PROTECTED_CACHE_SAFETY_EVIDENCE_REQUIRED`를 반환하고 confirm은 `503 TAKEDOWN_CONFIRM_DISABLED`입니다. 테스트 성공 경로만 production bean을 test-context mock으로 대체합니다.
...
| 영역 | 현재 패키지 | 역할 |
| --- | --- | --- |
| Web/scheduler/Kafka adapter | `publication.adapter.in.web`, `publication.adapter.in.scheduling`, `archive.adapter.in.web`, `feedback.adapter.in.web`, `session.adapter.in.web`, `sessionclosing.adapter.in.web`, `sessionimport.adapter.in.web`, `note.adapter.in.web`, `auth.adapter.in.web`, `notification.adapter.in.web`, `notification.adapter.in.scheduler`, `notification.adapter.in.kafka`, `admin.audit.adapter.in.web`, `admin.health.adapter.in.web`, `admin.operations.adapter.in.web`, `admin.takedown.adapter.in.web`, `aigen.adapter.in.web`, `aigen.adapter.in.messaging`, `aigen.adapter.in.scheduling`, `shared.adapter.in.web` | HTTP request validation, `CurrentMember` 주입, input use case 호출, scheduler trigger, Kafka listener dispatch, response mapping; shared health endpoint |
...
### Workflow side
- `publication`은 공개 projection read와 provider convergence workflow를 함께 소유합니다. Provider 작업은 mutable lease row만 claim/update하고, attempt event는 `PENDING` 뒤 terminal을 append-only로 기록합니다. Claim/PENDING transaction이 commit된 뒤에만 provider port를 호출하며 terminal은 별도 transaction에서 기록합니다.
- `(convergenceId, attemptNo)`에서 파생한 idempotency token은 crash replay 동안 동일하고 terminal failure retry만 더 높은 attempt를 사용합니다. Scheduler와 provider HTTP adapter는 기본 비활성이며 feature, scheduler, HTTP provider 세 토글이 모두 명시적으로 활성화된 경우에만 외부 호출을 시작합니다.
- `admin.takedown`은 platform capability, preview/confirm, admin-scoped HMAC idempotency, origin deny/generation rotation, immutable redacted receipt/audit와 초기 convergence work를 조정합니다. Provider 호출을 소유하지 않으며 production activation evidence adapter는 protected Step 8 증거가 연결될 때까지 항상 거절합니다.
...
운영 schema의 source of truth는 `server/src/main/resources/db/mysql/migration/`이다. 현재 catalog는 `V1`, `V9`~`V65`의 58개 versioned SQL이며 `V2`~`V8` gap은 의도적으로 유지한다. `V52`는 독립 session revision domain, `V53`은 host mutation idempotency/receipt, `V54`는 public projection convergence control plane, `V55`는 monotonic emergency deny marker와 platform-admin takedown evidence, `V56`은 convergence retention index, `V57`은 shared platform-admin command claim/alias/key-state substrate, `V58`은 platform-admin club preview/immutable receipt/convergence evidence를 추가한다. `V59`는 기존 notification confirmation을 snapshot semantics로 보존·backfill하고 notification·AI typed receipt와 provider attempt evidence를 추가하는 compatibility migration이다. `V60`은 support command evidence와 active-slot uniqueness를 추가하면서 legacy free-text reason을 비가역 redaction한다. `V61`은 schedule revision/participant seen/coarse club access, `V62`는 host workbox deferral/immutable snapshot, `V63`은 editable manual-notification copy snapshot, `V64`는 named invitation link, `V65`는 club settings/co-host/club-end command evidence를 각각 소유한다.
...
위반을 `flyway repair`, baseline 증가, 과거 파일 복원 없는 삭제·rename, 낮거나 재사용한 version으로 우회하지 않는다. checker가 보고한 base 최고 version보다 큰 새 `V{N}__{lower_snake_case_description}.sql` forward-only migration으로 보정한다. Testcontainers는 clean install과 legacy schema에서 V65까지의 upgrade를 유지하고, 합성 checksum fixture는 변조 거부와 정상 successor 적용을 검증한다.

V61–V65를 사용하는 frontend보다 해당 migration과 API를 포함한 backend를 먼저 시작한다. DB backup/snapshot 뒤 backend health와 `flyway_schema_history`의 V61–V65 성공을 확인하고 compatible frontend를 배포한다. 회귀 시 적용 SQL을 수정·삭제하거나 downgrade하지 않고 schema와 immutable snapshot/receipt/history를 읽을 수 있는 compatible image 또는 더 높은 version의 forward-fix를 사용한다. 이 순서는 repository contract이며 production rollout 완료 증거가 아니다.
...
Redis는 선택 계층이며 기본 설정에서는 꺼져 있습니다. Redis가 없어도 서버는 MySQL을 source of truth로 사용해 session, membership, publication, notes 데이터를 읽고 씁니다. AI generation처럼 Redis를 handoff/state store로 쓰는 기능은 별도 feature flag까지 켜진 환경에서만 동작합니다. Redis-backed 기능은 `READMATES_REDIS_ENABLED=true`와 기능별 flag를 함께 켰을 때만 동작합니다.
...
호스트 홈 `/app/host`는 모임 생애주기 운영실입니다(ADR-0048 Accepted). 서버가 고른 현재 모임을 준비실·현장·마감실로 나누고 다음 행동 하나, 준비 현황과 immutable-snapshot 작업함을 같은 문맥에 보여 주며, `OPEN`/`DRAFT`가 있어도 특정 모임으로 자동 redirect하지 않습니다. 모임 목록(`/app/host/sessions`)이 과거·예정 목차이고, 운영자가 보는 모임 lifecycle 단계는 모임 전(`DRAFT`), 진행 중(`OPEN`), 모임 후(`CLOSED` 또는 `PUBLISHED`)이며 서버 상태 이름은 `DRAFT`/`OPEN`/`CLOSED`/`PUBLISHED`를 유지합니다. 특정 모임 canonical 경로는 `/clubs/:slug/app/host/sessions/:sessionId`(등록 host의 `/app/host/sessions/:sessionId`)이며 page-level composition은 다이어리 스프레드입니다. `/edit`와 `/closing`은 canonical 경로로 redirect합니다. 새 모임 폼은 `GET /api/host/sessions/schedule-defaults`가 최근 최대 10모임의 반복 일정 값(시간·장소·접속 정보·질문 마감 오프셋·`accessScope`)을 채우며, 책 제목은 채우지 않습니다.
...
공개 effect mutation은 origin 반영과 provider convergence를 분리합니다. Immutable mutation receipt와 append-only attempt event는 수정하지 않고 `public_convergence_work`의 lease/cursor/availability만 갱신합니다. HTTP adapter 준비는 provider 플래그만 사용하지만 work 실행은 convergence feature에, scheduler 실행은 feature·scheduler·HTTP provider의 삼중 opt-in에 묶입니다. HTTP provider가 준비될 때 lease는 connect timeout, request/read timeout, 명시적 terminal-write safety margin의 합보다 엄격히 커야 하며, provider-neutral 계약은 2xx만 성공으로 인정합니다. 409를 포함한 다른 응답은 provider별 idempotent-duplicate 성공 계약이 문서화되기 전에는 bounded 실패입니다. 호스트 상태 API는 소속 클럽과 세션을 다시 확인하고 origin 결과, committed generation, bounded status/timestamp, retry 가능 여부만 반환합니다. Retry 가능 여부는 남은 attempt 예산이 있는 `FAILED`에만 참이고, raw provider 응답·오류·result category는 노출하지 않습니다.
...
특정 모임 화면은 `front/src/app/host-routes/meeting-route-element.tsx`가 `HostMeetingWorkspaceRoute`를 조립합니다. 순수 화면 모델은 `front/features/host/model/host-session-workspace-model.ts`, URL 해석은 `host-session-workspace-navigation.ts`가 소유합니다. Page-level 표시는 `front/features/host/ui/meeting-workspace/host-meeting-workspace.tsx`의 모임 상세이며 ADR-0048 운영실과 일정/기록 영역에서 같은 canonical route로 진입합니다. `HostSessionWorkspace`가 인접 모임 pager → 모임 identity·lifecycle timeline → `지금 할 일` → 단계 본문 → 관련 작업 → undo/recovery → panel/sheet 순서를 만들고, 기록 작업은 같은 workspace의 records panel 또는 embedded 마감 보드로 엽니다. 화면 상태 표기는 `작성 중`/`준비 중`/`기록 정리 중`/`게시됨`(`hostMeetingLifecycleLabel`)이고, 날짜는 권장 작업 우선순위만 바꿉니다. Record-dependent primary action은 기존 record editor query를 readiness union으로 읽으며 pending/stale/unavailable을 `false`로 추정하지 않습니다. `section` query는 active task/panel deep link이며 Back/Forward와 focus 복원을 지원합니다.
...
호스트 앱의 `/edit`와 `/closing`은 모임 canonical 경로 `/clubs/:slug/app/host/sessions/:sessionId`로 redirect합니다(`/closing`은 다이어리 기록 정리 단계 `section=records`로 착지). 모임 후 단계는 같은 다이어리에서 출석 수정, `정리본 올리기`, 기록 공개를 다룹니다. 서버의 `sessionclosing` slice는 새 영속 상태나 DB migration 없이 기존 세션·공개 기록·피드백 문서·알림 outbox/inbox 데이터를 읽어 checklist, next action, Host/Member/Public surface 상태, evidence ledger를 계산합니다.
...
호스트 primary route는 운영실 `/app/host`, 일정과 모임 `/app/host/sessions`, 사람 `/app/host/people`, 기록 `/app/host/records`이며 scoped route는 앞에 `/clubs/:slug`를 붙입니다. `/app/host/settings`, `/notifications`, `/sessions/new`는 utility/action입니다. `/members`와 `/operations`는 query·허용된 incoming fragment·검증된 same-club return state를 보존해 각각 `/people`과 운영실로 replace합니다. `/invitations`는 query와 검증된 same-club return state를 보존하되 incoming fragment를 canonical `#invitations`로 교체해 `/settings#invitations`로 replace합니다. `/records`는 redirect가 아니라 canonical 기록 원장입니다. Route inventory와 continuity는 `front/src/app/route-continuity.ts`, `front/src/app/routes/host.test.tsx`, `front/tests/e2e/host-lifecycle-route-continuity.spec.ts`가 소유합니다.

운영실의 서버 composition은 `com.readmates.hostworkspace`가 소유합니다. `hostworkspace.application`과 `hostworkspace.domain`은 다른 feature를 import하지 않고, `adapter.out.source`만 다른 feature의 input port를 호출하며 foreign adapter/persistence를 직접 참조하지 않습니다. 이 one-way rule은 `ServerArchitectureBoundaryTest`와 `ServerArchitectureInventoryTest`가 강제합니다. 현재 모임 selection, workbox, 사람 상세는 hostworkspace input port로 노출되고 각 source domain 사실은 outbound adapter 뒤에 남습니다.

일정 확인 write는 loader/prefetch가 아니라 ACTIVE participant가 `OPEN` current schedule을 실제 렌더링한 뒤 exact `scheduleRevision`에 대해 한 번 시작합니다. 같은 revision retry는 최초 timestamp를 보존하고, stale/future revision은 `409`, removed/inactive/cross-club/DRAFT는 fail closed합니다. RSVP, 실제 출석, notification row, domain cursor epoch는 이 write로 바뀌지 않고 seen 실패도 RSVP·질문 등 나머지 member action을 막지 않습니다. 호스트 projection은 `CURRENT|STALE|UNSEEN`, current revision과 확인 시각만 제공하며 email, account ID, page/action history를 제공하지 않습니다. `lastClubAccessAt`은 ACTIVE membership의 coarse 사실로만 별도 유지합니다. 구현/검증 source는 `front/features/current-session/route/current-session-route.tsx`, `SessionScheduleSeenDbTest`, `FrontendZodSchemaContractTest`, `schedule-seen-lifecycle.spec.ts`입니다.

작업함의 first page는 다섯 source(`SCHEDULE_UNSEEN`, `MEMBER_APPROVAL`, `RECORD_CLOSING`, `INVITATION_EXPIRY`, `NOTIFICATION_FAILURE`)를 한 `REPEATABLE_READ` 평가에서 읽어 host/club/state/filter에 결속된 15분 immutable snapshot을 저장합니다. Continuation은 그 snapshot만 읽어 source 동시 변경에도 no-gap/no-duplicate를 유지하고, 새 first page만 새 source 상태를 봅니다. 보류만 host-owned mutable fact이며 만료되면 `NOW`로 돌아옵니다. `COMPLETED`는 별도 flag가 아니라 source transition 또는 allowlist receipt에서 파생하고 30일 bounded projection만 제공합니다. Raw email/token/provider body/page history/user ID는 snapshot에 넣지 않습니다. `HostWorkboxServiceTest`, `JdbcHostWorkboxAdapterTest`, `JdbcHostWorkSourceAuthorityTest`가 이 경계를 검증합니다.

일정 미열람 수동 알림은 editable subject/body, exact target snapshot, schedule revision과 content hash를 preview에 묶습니다. Confirm은 idempotency key로 한 dispatch/receipt에 수렴하고, transport response가 사라지면 blind resend하지 않고 dispatch/event/history를 조회해 결과를 reconcile합니다. Partial source failure는 성공한 sibling을 유지하고 실패 source만 retry합니다. Frontend contract와 recovery는 `host-schedule-review-route.tsx`, `host-workbox-stage4.spec.ts`, server idempotency는 `HostManualNotificationServiceTest`와 `HostNotificationControllerTest`가 검증합니다. 실제 provider/email 전달은 repository evidence가 아닙니다.

`/people/:membershipId`는 coarse access, schedule seen, RSVP, actual attendance를 별도 projection으로 제공합니다. `/settings`의 named invitation link는 생성 시 share path를 한 번만 반환하고 raw token을 저장하지 않으며, 기존 OAuth invitation flow에서 같은 club의 ACTIVE MEMBER로만 원자 소비되고 HOST를 부여하지 않습니다. Club settings/co-host/운영 종료 command는 revision, history, idempotency와 preview/confirm을 사용합니다. `NamedInvitationLinkOAuthDbTest`, `HostClubSettingsConcurrencyDbTest`와 frontend Zod contract tests가 local-safe contract를 검증하며 외부 OAuth/provider와 실제 클럽 종료는 `not measured`입니다.

## 이메일 알림, 멤버 알림함, 호스트 운영

멤버 알림 설정은 `/api/me/notifications/preferences`에서 읽고 저장합니다. 선호도 row가 없으면 운영 알림(`NEXT_BOOK_PUBLISHED`, `SESSION_REMINDER_DUE`, `FEEDBACK_DOCUMENT_PUBLISHED`, `SESSION_RECORD_UPDATED`)은 켜짐, 서평 공개 알림(`REVIEW_PUBLISHED`)은 꺼짐으로 해석합니다. `FEEDBACK_DOCUMENT_PUBLISHED`와 `SESSION_RECORD_UPDATED`는 같은 `feedback_document_published_enabled` 선호도를 공유합니다. `/app/notifications/settings`는 정식 멤버와 호스트에게 이 설정을 보여주며, 둘러보기 멤버는 저장 가능한 알림 설정을 보지 않습니다.

멤버 알림함은 `/api/me/notifications`, `/api/me/notifications/unread-count`, `/api/me/notifications/{id}/read`, `/api/me/notifications/read-all`을 사용합니다. `/app/notifications`는 `member_notifications`를 source of truth로 읽고, unread count, 개별 읽음 처리, 전체 읽음 처리를 제공합니다. `/app/notifications/settings`와는 `받은 알림`·`수신 설정` tab으로 연결합니다. 알림 route는 `/app/me`의 utility 목록에서 진입하고, 두 route 모두 현재 club scope를 보존하는 고정 `내 공간` 상위 경로를 제공하며 영구 앱 내비게이션에서도 `내 공간` current state를 유지합니다. 각 알림의 deep link를 열면 해당 알림을 읽음 처리한 뒤 대상 화면으로 이동합니다.
...
외부 JSON 흐름은 [세션 기록 JSON 가져오기](#세션-기록-json-가져오기) 섹션에서 설명합니다. 현재 provider, 호출·비용·복구, trace/privacy 구조는 [Spring AI 2 provider architecture](spring-ai-2-provider-architecture.md), 운영 절차는 [AI 세션 생성 runbook](../operations/runbooks/ai-session-generation.md)을 기준으로 합니다.
