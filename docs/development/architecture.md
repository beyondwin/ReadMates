...
| 멤버 앱 | `/app`, `/app/pending`, `/app/session/current`, `/app/notes`, `/app/archive`, `/app/sessions/:sessionId`, `/app/feedback/:sessionId`, `/app/feedback/:sessionId/print`, `/app/me` | 둘러보기 멤버, 정식 멤버, 호스트 | 현재 세션 확인, 멤버 공개 예정 세션 확인, 둘러보기 멤버 안내, RSVP, 읽은 분량, 질문, 한줄평, 장문 서평, 아카이브, 참석 회차 피드백 문서, 본인 표시 이름과 알림 설정 변경 |
