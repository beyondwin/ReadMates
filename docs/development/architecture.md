...
운영 schema의 source of truth는 `server/src/main/resources/db/mysql/migration/`이다. 현재 catalog는 `V1`, `V9`~`V55`의 48개 versioned SQL이며 `V2`~`V8` gap은 의도적으로 유지한다. `V52`는 독립 session revision domain, `V53`은 host mutation idempotency/receipt, `V54`는 public projection convergence control plane, `V55`는 monotonic emergency deny marker와 platform-admin takedown evidence를 additive하게 추가한다.
...
`/api/public/clubs/{slug}`, `/api/public/clubs/{slug}/sessions/{sessionId}` 등 `PublicController`의 GET endpoint는 `Cache-Control: public, max-age=60, must-revalidate`를 응답 헤더에 포함합니다. Club list/stats/detail과 guest record의 공개 projection reader는 publication의 exact current generation row와 `origin_readable=true`를 함께 요구하므로 takedown transaction 뒤 origin에서 summary, book metadata, record와 count를 반환하지 않습니다. CDN 또는 브라우저는 새 header 기준 최대 60초 동안 fresh response를 사용할 수 있고, 기존 720초 header를 가진 browser 경계는 protected evidence gate로 별도 소진해야 합니다.
