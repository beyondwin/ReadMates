...
| 호스트 앱 | `/clubs/:slug/app/host`, `/clubs/:slug/app/host/notifications`, `/clubs/:slug/app/host/members`, `/clubs/:slug/app/host/sessions`, `/clubs/:slug/app/host/sessions/new`, `/clubs/:slug/app/host/sessions/:sessionId`, 등록된 club host의 `/app/host/**` | 현재 클럽의 호스트 | 1차 내비게이션은 3탭(오늘·모임·멤버)입니다(ADR-0046). `/app/host`는 처리할 일 큐와 다음 모임 히어로의 오늘 트리아지 홈이고, `OPEN`/`DRAFT`가 있어도 홈으로 머무릅니다. `/app/host/sessions`는 모임 목록(목차)이며, 특정 모임 `/sessions/:sessionId`는 생성→준비→당일→기록→게시 다이어리 스프레드입니다. `/edit`와 `/closing`은 같은 canonical 경로로 redirect합니다. 초대는 `/members`에 흡수되고, 구 URL `/records`→`/sessions`, `/operations`→오늘, `/invitations`→`/members`는 replace redirect입니다. 알림 발송(`/notifications`)은 1차 탭이 아닌 화면입니다. 다음 책 여러 권 미리 넣기, 공개 범위, 출석 확정, 모임 마치기, `정리본 올리기`·검토·적용·revision 복원, 기록 공개, 변경 되돌리기·7일 휴지통, 멤버 상태·표시 이름, 알림 발송을 제공합니다 |
