...
`ServerArchitectureInventoryTest`는 승인된 39개 boundary identity를 current 4개와 retired 35개로 정확히 분할하고, 승인된 41개 application feature dependency를 current 40개와 retired 1개로 고정합니다. 두 ledger는 목표 architecture가 아니라 제거 가능한 debt inventory입니다. Actor/auth–club 전환은 19개 boundary identity와 `club|auth`를 retired로 옮겼지만 baseline zero나 전체 Phase 2 완료를 뜻하지 않습니다. 현재 boundary row는 `sessionclosing` 1개, `sessionimport` 1개, `sessionrecord` 2개이며, feature graph의 유일한 cyclic component는 `session`/`sessionimport`/`sessionrecord`입니다. 이 후속 session-family 전환 전까지 `auth|club`과 `com.readmates.shared.adapter.in.web` 공통 web-error contract는 유지합니다. 이 경계 정리는 REST/API shape, authorization 의미, BFF/frontend source, migration/schema, deployment 또는 production behavior를 바꾸지 않았습니다.
