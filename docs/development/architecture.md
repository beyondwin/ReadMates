...
| 영역 | 현재 패키지 | 역할 |
| --- | --- | --- |
| Web/scheduler/Kafka adapter | `publication.adapter.in.web`, `archive.adapter.in.web`, `feedback.adapter.in.web`, `session.adapter.in.web`, `sessionclosing.adapter.in.web`, `sessionimport.adapter.in.web`, `note.adapter.in.web`, `auth.adapter.in.web`, `notification.adapter.in.web`, `notification.adapter.in.scheduler`, `notification.adapter.in.kafka`, `admin.audit.adapter.in.web`, `admin.health.adapter.in.web`, `admin.operations.adapter.in.web`, `aigen.adapter.in.web`, `aigen.adapter.in.messaging`, `shared.adapter.in.web` | HTTP request validation, `CurrentMember` 주입, use case 호출, scheduler trigger, Kafka listener dispatch, response mapping; shared health endpoint |
...
`ServerArchitectureInventoryTest`는 기존 boundary import를 최대 39개, application feature dependency edge를 최대 41개로 고정하고 현재 source와 baseline 파일이 정확히 일치하도록 검증합니다. 두 baseline은 승인된 목표 architecture가 아니라 제거 가능한 debt inventory입니다. Phase 0은 이 debt의 증가를 막고, actor·cross-adapter·feature-cycle debt의 실제 제거와 대응 baseline row 삭제는 Phase 2가 담당합니다. `com.readmates.shared.adapter.in.web`은 그때까지 공통 web-error contract로 유지합니다.
