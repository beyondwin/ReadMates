...
Platform admin 긴급 공개 회수는 `/api/admin/public-takedowns/preview|confirm`의 별도 `admin.takedown` workflow slice가 소유합니다. Active platform-admin 조회를 통과한 actor 중 `PlatformCapability.EMERGENCY_PUBLIC_TAKEDOWN`이 있는 OWNER/OPERATOR policy mapping만 preview를 만들 수 있고 application service는 role enum이 아니라 `actor.can(...)`으로 권한을 검사합니다. Preview는 actor role snapshot, club/session/publication UUID, generation, origin/BFF/CDN/browser 표면, 10분 TTL과 이미 저장·표시·offline인 사본을 원격 삭제할 수 없다는 경계를 고정합니다. Confirm은 동일 actor/role, TTL, current generation/surfaces, bounded category/reason/key를 다시 확인합니다.

V55의 admin idempotency scope는 `(platformAdminUserId, EMERGENCY_PUBLIC_TAKEDOWN, clubId, publicationId, key)`이며, reason 원문은 기존 secret-keyed request identity primitive로 HMAC한 뒤 versioned canonical request HMAC에만 포함합니다. 같은 key/request는 immutable receipt를 반환하고 다른 request는 conflict입니다. 새 confirm 한 번은 application-service transaction에서 origin을 deny하고 generation을 정확히 한 번 회전한 뒤 redacted admin receipt, `platform_audit_events.EMERGENCY_PUBLIC_TAKEDOWN_CONFIRMED`, V54 convergence link/work를 한 번씩 기록합니다. BFF eviction은 `NOT_STARTED`, CDN은 `QUEUED`, browser는 cache-policy revalidation로 구분하며 provider attempt는 transaction 밖 Task 2 convergence processor가 같은 `convergenceId` 아래 append-only로 추가합니다. Preview/idempotency row는 bounded retention 대상이고 content FK가 없는 redacted admin/convergence evidence는 session/publication hard delete 뒤에도 남습니다.
