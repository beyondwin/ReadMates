...
| 호스트 앱 | `/app/host`, `/app/host/members`, `/app/host/invitations`, `/app/host/sessions/new`, `/app/host/sessions/:sessionId/edit` | 호스트 | 예정 세션 생성/수정, 공개 범위 설정, 현재 세션 시작, 참석 확정, 공개 기록 발행, 초대 관리, 멤버 상태와 표시 이름 관리, 피드백 문서 업로드 |
