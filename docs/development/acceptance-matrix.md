...
| Session lifecycle | `DRAFT` → `OPEN` → `CLOSED` → `PUBLISHED`, one `OPEN` per club, idempotent open/close, no revert from `PUBLISHED` | Allowed and rejected transition evidence |
