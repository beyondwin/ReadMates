...
| BFF or OAuth | same-origin proxy, cookie/session ID rotation, safe exact raw return path, POST-issued expiring one-use join intent, exact provider-state binding across multiple tabs, reverse-order success/failure with remaining-state survival, crafted GET/replay/mismatch rejection, raw invite-parameter priority, target-club join, trusted header stripping | BFF unit test, actual success/failure handler plus OAuth repository/integration test, and relevant E2E flow |
