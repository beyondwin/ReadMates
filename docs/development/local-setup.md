...
- Dev seed와 E2E fixture는 현재 `OPEN` 세션, `CLOSED`/`PUBLISHED` 기록, 호스트가 새로 만드는 `DRAFT` 예정 모임 흐름을 검증할 수 있게 구성되어 있습니다. 호스트는 `/app/host`에서 예정 모임의 멤버에게 보이기를 바꾸고, `DRAFT`를 진행 중으로 연 뒤 `/app/host/sessions/:sessionId`에서 모임 마치기와 기록 공개 흐름을 확인할 수 있습니다.
