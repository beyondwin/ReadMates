...
- local runtime/tool state, including `.waygent/` orchestration/cache state
...
4. 변경 전에는 루트 `AGENTS.md`가 지시하는 surface guide를 읽습니다.
5. Graphify 결과와 실제 코드가 충돌하면 실제 코드, 테스트, migration, active docs를 우선합니다.
6. Release readiness나 residual risk review는 Graphify query가 아니라 branch diff와 `docs/development/release-readiness-review.md`를 기준으로 합니다.
