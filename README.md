...
| 플랫폼 관리자 | `/admin/today` 트리아지 · `/admin/clubs[/{clubId}]` 클럽 운영 · `/admin/ai-ops` AI Ops · `/admin/support` 지원 라우트 패밀리에서 좌측 nav, 상단 status strip, OWNER/OPERATOR/SUPPORT 권한 매트릭스로 플랫폼 운영을 합니다. 클럽별 호스트/멤버 권한과 별도 권한입니다. |
