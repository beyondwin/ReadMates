...
- 알림 운영: 멤버는 `/app/me`에서 이메일 알림과 이벤트별 수신 여부를 저장하고, `/app/notifications`에서 in-app 알림을 읽거나 모두 읽음 처리할 수 있습니다. 호스트는 `/app/host/notifications`에서 event publication row와 channel delivery row를 확인하고, email delivery의 pending/failed 처리와 `DEAD` 복구, 고정 템플릿 테스트 메일과 audit 확인을 수행합니다. 테스트 메일 audit은 masked email과 hash만 저장합니다.
