...
private typealias ManualConfirmAttempt = ManualNotificationConfirmAttempt
...
class CanonicalMeetingLanguageInventoryTest {
    ...
    private fun hostManualNotificationTemplateLabels(): Map<NotificationEventType, String> {
        val source =
            Files.readString(
                projectRoot().resolve(
                    "server/src/main/kotlin/com/readmates/notification/application/service/" +
                        "HostManualNotificationService.kt",
                ),
            )
    ...
    private class CopyInventoryManualPort : ManualNotificationDispatchPort {
        ...
        override fun confirmManualDispatch(input: ManualNotificationConfirmTransactionInput): ManualConfirmAttempt {
            val message = "unused"
    ...
    private class CopyInventoryImportPort(
        ...
        override fun replaceRecords(command: SessionImportRecordReplacement): StoredFeedbackDocument = error("unused")
    }
}
