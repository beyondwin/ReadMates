...
class AuthMeControllerTest(
    ...
    fun `returns seeded member payload when authenticated email has active membership`() {
        mockMvc
            .get("/api/auth/me") {
                ...
                jsonPath("$.currentMembership.avatarKey") { value("apple-green-book") }
    ...
    private fun loginAsGoogleViewerUser(email: String): Cookie {
        ...
        jdbcTemplate.update(
            ...
            select ?, clubs.id, ?, 'MEMBER', 'VIEWER', null, ?, 'mushroom-green-book'
            from clubs
    ...
    private fun insertSampleClubMembershipForMember5(): String {
        ...
        jdbcTemplate.update(
            """
            insert into memberships (id, club_id, user_id, role, status, joined_at, short_name, avatar_key)
            select ?, clubs.id, users.id, 'MEMBER', 'ACTIVE', utc_timestamp(6), '샘플멤버5', 'mushroom-green-book'
            from clubs
    ...
    private fun insertSampleClubMembershipForHost(role: String): String {
        ...
        jdbcTemplate.update(
            """
            insert into memberships (id, club_id, user_id, role, status, joined_at, short_name, avatar_key)
            select ?, clubs.id, users.id, ?, 'ACTIVE', utc_timestamp(6), '샘플호스트', 'mushroom-green-book'
            from clubs
    ...
    private fun loginAsLifecycleUser(
        ...
        jdbcTemplate.update(
            """
            insert into memberships (id, club_id, user_id, role, status, joined_at, short_name, avatar_key)
            select ?, clubs.id, ?, 'MEMBER', ?, utc_timestamp(6), ?, 'mushroom-green-book'
            from clubs
            where clubs.slug = 'reading-sai'
            """.trimIndent(),
            membershipId,
