...
class NamedInvitationLinkOAuthDbTest(
    ...
    fun `named link preview is redacted and oauth acceptance creates member once without redisclosure`() {
        ...
        assertNamedAcceptancePersistedOnce(created.link.linkId)
    }

    private fun assertNamedAcceptancePersistedOnce(linkId: UUID) {
        assertThat(
            jdbc.queryForObject(
        ...
        val acceptedHashes =
            jdbc.queryForMap(
                """
                select idempotency_key_hash, request_hash from host_invitation_link_events
                where link_id = ? and action = 'ACCEPTED'
                """.trimIndent(),
                linkId.toString(),
            )
        assertThat(acceptedHashes.values.map { it.toString() })
            .doesNotContain(
                com.readmates.shared.security.TokenHashing
                    .sha256("named-accept:$linkId:task4-named-google"),
                com.readmates.shared.security.TokenHashing
                    .sha256("named-accept:$linkId:task4.named@example.com"),
            )
    }
    ...
    fun `concurrent identical create commands converge to one event and one disclosed path`() {
        ...
        assertThat(
            jdbc.queryForObject(
                "select count(*) from host_invitation_links where name = '동시 생성 링크'",
                Int::class.java,
            ),
        ).isEqualTo(1)
        assertThat(
            jdbc.queryForObject(
                "select count(*) from host_invitation_link_events where link_id = ?",
                Int::class.java,
                values
                    .first()
                    .link.linkId
    ...
    fun `concurrent identical updates converge while conflicting canonical command conflicts`() {
        ...
        val command =
            UpdateHostInvitationLinkCommand(
                ...
                "동시 수정 완료",
                5,
                created.link.expiresAt.plusDays(1),
        ...
        assertThat(
            jdbc.queryForObject(
                "select count(*) from host_invitation_link_events where link_id = ? and action = 'UPDATED'",
                Int::class.java,
    ...
    fun `paused and cross club named links fail before identity or membership mutation`() {
        ...
        assertThat(
            jdbc.queryForObject(
                "select count(*) from users where email = 'task4.paused@example.com'",
                Int::class.java,
            ),
        ).isZero()
    }
