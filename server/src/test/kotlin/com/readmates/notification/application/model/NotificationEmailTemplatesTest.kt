...
class NotificationEmailTemplatesTest {
    ...
    fun `renders next book email with editorial ledger structure and absolute cta`() {
        ...
        assertThat(copy.title).isEqualTo("No.8 책이 정해졌습니다")
        assertThat(copy.body).isEqualTo("No.8 Distributed Systems 책이 정해졌습니다.")
        assertThat(copy.deepLinkPath).isEqualTo("/clubs/reading-sai/app")
        assertThat(copy.emailSubject).isEqualTo("No.8 책이 정해졌습니다")
        assertThat(copy.emailBodyText).contains(
            "읽는사이",
            "민서님,",
            "다음 모임에서 함께 읽을 책이 정해졌습니다.",
            "모임 전과 준비 내용을 확인해 주세요.",
            "모임: No.8",
