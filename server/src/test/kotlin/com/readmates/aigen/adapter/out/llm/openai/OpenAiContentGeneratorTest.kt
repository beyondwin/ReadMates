...
class OpenAiContentGeneratorTest {
    ...
    fun `provider field is OPENAI`() {
        val generator = OpenAiContentGenerator(FakeOpenAiApi(), schemaResource)
        assertEquals(Provider.OPENAI, generator.provider)
    }

    @Test
    fun `passes full SessionImportV1 schema to json_schema`() {
        val fake = FakeOpenAiApi(result = OpenAiToolResult(validSnapshotJson(), zeroUsage()))
        val generator = OpenAiContentGenerator(fake, schemaResource)

        generator.generateFull(input)

        assertEquals(schemaResource.schema(), fake.lastSchema)
    }

    @Test
    fun `passes the configured schema name session_import_v1`() {
        val fake = FakeOpenAiApi(result = OpenAiToolResult(validSnapshotJson(), zeroUsage()))
        val generator = OpenAiContentGenerator(fake, schemaResource)

        generator.generateFull(input)

        assertEquals("session_import_v1", fake.lastSchemaName)
    }
    ...
    @Test
    fun `USER_PRELUDE matches Claude generator user prelude verbatim`() {
        val fake = FakeOpenAiApi(result = OpenAiToolResult(validSnapshotJson(), zeroUsage()))
        val generator = OpenAiContentGenerator(fake, schemaResource)

        generator.generateFull(input)

        // Identical system + user prelude is part of the cross-provider contract:
        // generation results must not vary because of accidental prompt drift.
        val expected =
    ...
    fun `rate-limited upstream exception maps to PROVIDER_RATE_LIMITED without leaking sentinel`() {
        val sentinel = "UNIQUE-SENTINEL-12345"
        val fake = FakeOpenAiApi(throwException = RuntimeException("rate_limit 429 around $sentinel"))
        val generator = OpenAiContentGenerator(fake, schemaResource)
