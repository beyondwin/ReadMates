...
class OpenAiContentRegeneratorTest {
    ...
    @Test
    fun `provider field is OPENAI`() {
        val regen = OpenAiContentRegenerator(FakeOpenAiApi(), schemaResource)
        assertEquals(Provider.OPENAI, regen.provider)
    }
    ...
    fun `SUMMARY uses narrowed schema with only summary property and returns String value`() {
        ...
        val fake = FakeOpenAiApi(result = OpenAiToolResult(toolInput, TokenUsage(10, 0, 20)))
        val regen = OpenAiContentRegenerator(fake, schemaResource)
        ...
        assertEquals("session_import_v1_summary", fake.lastSchemaName)
    }
    ...
    fun `HIGHLIGHTS uses narrowed schema with only highlights array and returns List of AuthoredText`() {
        ...
        val fake = FakeOpenAiApi(result = OpenAiToolResult(toolInput, TokenUsage(0, 0, 0)))
        val regen = OpenAiContentRegenerator(fake, schemaResource)
        ...
        val props = fake.lastSchema!!.path("properties")
        assertTrue(props.has("highlights"))
        assertEquals(1, props.size())
        assertEquals("array", props.path("highlights").path("type").asText())
        assertEquals("session_import_v1_highlights", fake.lastSchemaName)
    }
    ...
    fun `ONE_LINE_REVIEWS uses narrowed schema and returns List of AuthoredText`() {
        ...
        assertEquals(1, props.size())
        assertEquals("session_import_v1_oneLineReviews", fake.lastSchemaName)
    }

    @Test
    fun `FEEDBACK_DOCUMENT uses narrowed schema with both feedback fields and returns FeedbackDocumentValue`() {
        val json =
            "{" +
        ...
        val value = output.patchedValue as OpenAiContentRegenerator.FeedbackDocumentValue
        assertEquals("new.md", value.fileName)
        assertTrue(value.markdown.contains("<!-- readmates-feedback:v1 -->"))

        val schema = fake.lastSchema!!
        ...
        assertEquals("session_import_v1_feedbackDocument", fake.lastSchemaName)
    }
    ...
    @Test
    fun `forwards model id name to api port`() {
        val toolInput = mapper.readTree("""{"summary":"x"}""") as ObjectNode
        val fake = FakeOpenAiApi(result = OpenAiToolResult(toolInput, TokenUsage(0, 0, 0)))
        val regen = OpenAiContentRegenerator(fake, schemaResource)
    ...
    fun `PII invariant - wrapped error message never echoes sentinel from IOException`() {
        ...
        val fake = FakeOpenAiApi(throwException = IOException("read failed near $sentinel"))
        val regen = OpenAiContentRegenerator(fake, schemaResource)
