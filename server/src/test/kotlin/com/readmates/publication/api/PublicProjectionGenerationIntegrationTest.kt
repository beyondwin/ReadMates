...
import com.readmates.club.application.service.PlatformAdminClubVisibilityService
...
class PublicProjectionGenerationIntegrationTest(
    ...
    @Autowired private val platformClubVisibility: PlatformAdminClubVisibilityService,
        ...
        try {
            repeat(4) { iteration ->
                ...
                try {
                    ...
                    val previews =
                        (1..2).map {
                            platformClubVisibility.previewVisibility(
                                admin,
                                fixture.clubId,
                                PreviewPlatformAdminClubVisibilityCommand(0, ClubPublicVisibility.PRIVATE),
                            )
                        }
                    val updates =
                        (1..2).map { attempt ->
                            executor.submit<Result<ClubPublicVisibility>> {
                                ready.countDown()
                                check(start.await(5, TimeUnit.SECONDS))
                                runCatching {
                                    platformClubVisibility.confirmVisibility(
                                        admin,
                                        fixture.clubId,
                                        ConfirmPlatformAdminClubVisibilityCommand(
                                            previews[attempt - 1].previewId,
                                            "fixture-concurrent-visibility-$iteration-$attempt",
                                            0,
                    ...
                    val outcomes = updates.map { it.get(10, TimeUnit.SECONDS) }
                    assertThat(outcomes.count(Result<ClubPublicVisibility>::isSuccess)).isEqualTo(1)
                    assertThat(outcomes.count(Result<ClubPublicVisibility>::isFailure)).isEqualTo(1)
                    val after = currentMarker(fixture.sessionId.toString())
                    assertThat(after.first - before.first).isEqualTo(1)
                    ...
                    val clubExposure =
                        executor.submit<ClubPublicVisibility> {
                            ...
                            platformClubVisibility.confirmVisibility(
                                ...
                                fixture.clubId,
                                ConfirmPlatformAdminClubVisibilityCommand(
                                    exposurePreview.previewId,
                                    "fixture-exposure-basic-$iteration",
                                    0,
                                    ClubPublicVisibility.PRIVATE,
                                    true,
                                ),
    ...
    private fun makeClubPrivate(fixture: ConcurrencyFixture): ClubPublicVisibility =
        adminActor().let { admin ->
            ...
            platformClubVisibility.confirmVisibility(
                ...
                ConfirmPlatformAdminClubVisibilityCommand(
                    preview.previewId,
                    "fixture-private-${fixture.clubId}",
                    0,
                    ClubPublicVisibility.PRIVATE,
                    true,
                ),
