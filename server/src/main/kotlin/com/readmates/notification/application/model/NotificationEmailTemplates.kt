...
object NotificationEmailTemplates {
    ...
    private fun htmlEmail(
        ...
        val cta =
            if (!ctaLabel.isNullOrBlank() && !ctaUrl.isNullOrBlank()) {
                """
                <tr>
                  <td style="padding:28px 0 0;">
                    <a href="${escapeHtml(
                    ctaUrl,
                )}" style="display:block;padding:14px 18px 15px;border:1px solid #1f3b61;background:#27466f;color:#f9f3e9;text-align:center;text-decoration:none;font-size:14px;font-weight:700;">${escapeHtml(
                    ctaLabel,
                )}</a>
                  </td>
                </tr>
        ...
        val rows =
            contextRows.joinToString("") { (rowLabel, rowValue) ->
                """
                <tr>
                  <td style="padding:8px 12px 8px 0;border-top:1px solid #e9ded0;color:#687084;font-size:14px;line-height:1.45;vertical-align:top;width:82px;">${escapeHtml(
                    rowLabel,
                )}</td>
                  <td style="padding:8px 0;border-top:1px solid #e9ded0;color:#17233a;font-size:14px;line-height:1.45;font-weight:700;vertical-align:top;">${escapeHtml(
                    rowValue,
                )}</td>
                </tr>
                """.trimIndent()
    ...
    private fun absoluteUrl(
        appBaseUrl: String,
        path: String,
    ): String = "${appOrigin(appBaseUrl)}${path.ensureLeadingSlash()}"
