...
private val ERROR_PRIVATE_KEY_BLOCK_PATTERN =
    Regex(
        """(?is)-----BEGIN [^-]*(?:PRIVATE KEY|OPENSSH PRIVATE KEY|RSA PRIVATE KEY)[\s\S]*?(?:-----END [^-]*(?:PRIVATE KEY|OPENSSH PRIVATE KEY|RSA PRIVATE KEY)-----|$)""",
    )
...
private val ERROR_SECRET_ASSIGNMENT_PATTERN =
    Regex(
        """(?i)\b(?:password|passwd|pwd|passcode|secret|api[-_ ]?key|access[-_ ]?key|secret[-_ ]?key|client[-_ ]?secret|private[-_ ]?key|token|credential)\s*[:=]\s*(?:"[^"]*"|'[^']*'|[^\s,;]+)""",
    )
private val ERROR_SECRET_KEYWORD_VALUE_PATTERN =
    Regex(
        """(?i)\b(?:password|passwd|pwd|passcode|secret|api[-_ ]?key|access[-_ ]?key|secret[-_ ]?key|client[-_ ]?secret|private[-_ ]?key|token|credential)\s+(?:is\s+|was\s+)?(?:"[^"]*"|'[^']*'|[^\s,;]+)""",
    )
