...
class JdbcHostInvitationLinkStoreAdapter(
    ...
    override fun update(
        ...
        if (changed !=
            ...
            throw InvitationDomainException("INVITATION_LINK_STALE", InvitationDomainError.CONFLICT, "Invitation link changed")
        }
        insertEvent(event)
    }
    ...
    override fun consume(
        ...
        if (changed !=
            ...
            throw InvitationDomainException("INVITATION_LINK_EXHAUSTED", InvitationDomainError.CONFLICT, "Invitation link is exhausted")
        }
        insertEvent(event)
        return jdbcTemplate
            .query(
...
private data class LinkCursor(
    ...
    companion object {
        fun from(cursor: Map<String, String>): LinkCursor? {
            val created = cursor["createdAt"]?.let { runCatching { OffsetDateTime.parse(it) }.getOrNull() } ?: return null
            val id = cursor["id"]?.takeIf(String::isNotBlank) ?: return null
            return LinkCursor(created, id)
        }
    }
}
