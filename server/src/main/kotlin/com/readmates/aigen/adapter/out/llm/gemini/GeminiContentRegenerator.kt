...
/**
 * Gemini provider implementation of [SessionContentRegenerator].
 *
 * Narrows the `readmates-session-import:v1` schema down to just the
 * field(s) corresponding to the requested [GenerationItem], converts
...
class GeminiContentRegenerator(
    ...
    override fun regenerateItem(input: RegenerationInput): RegenerationOutput {
        ...
        val geminiNarrowed = schemaAdapter.convert(narrowed)
     ...
     *
     * The narrowed schema still carries `additionalProperties = false`
     * (matching OpenAi's contract) — [GeminiSchemaCompatAdapter] strips
     * it later when converting to Gemini's OpenAPI 3.0 subset.
     */
