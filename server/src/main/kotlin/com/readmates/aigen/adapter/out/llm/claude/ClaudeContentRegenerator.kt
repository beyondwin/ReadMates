...
/**
 * Claude provider implementation of [SessionContentRegenerator].
 *
 * Narrows the `readmates-session-import:v1` tool schema down to just
 * the field(s) corresponding to the requested [GenerationItem], forces
 * the model to invoke `emit_session_import_v1_partial` with that
 * narrowed schema, then parses the resulting tool input into the
 ...
 *
 * Gated behind `readmates.aigen.enabled=true`.
 */
@Component
...
class ClaudeContentRegenerator(
    ...
    override fun regenerateItem(input: RegenerationInput): RegenerationOutput {
        ...
        val patchedValue = parseValue(result.input, input.item)
    ...
    companion object {
        const val TOOL_NAME: String = "emit_session_import_v1_partial"
    }
}
