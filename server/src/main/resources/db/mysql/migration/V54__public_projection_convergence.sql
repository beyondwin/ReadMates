...
create table auth_public_projection_mutation_receipts (
  ...
  constraint auth_public_projection_receipts_operation_check check (
    trim(operation) <> ''
  )
) default character set utf8mb4 collate utf8mb4_0900_ai_ci;
...
create table public_convergence_work (
  ...
  key public_convergence_work_claim_idx (available_at, lease_expires_at, created_at),
  key public_convergence_work_retention_idx (retention_until),
  ...
  constraint public_convergence_work_lease_check check (
    ...
    or (lease_owner is not null and trim(lease_owner) <> '' and lease_expires_at is not null)
  )
) default character set utf8mb4 collate utf8mb4_0900_ai_ci;
...
create table public_convergence_events (
  ...
  key public_convergence_events_resource_idx (
    club_id_snapshot, session_id_snapshot, observed_at desc
  ),
  ...
  constraint public_convergence_events_sequence_check check (
    (
      event_seq = 0
      and binary status = binary 'PENDING'
...
select ranked.convergence_id,
       ranked.attempt_no,
       ranked.event_seq,
       ranked.status,
       ranked.observed_at,
       ranked.result_category,
       ranked.club_id_snapshot,
       ranked.session_id_snapshot,
       ranked.publication_id_snapshot
from (
  select events.*,
         row_number() over (
           partition by events.convergence_id
           order by events.attempt_no desc, events.event_seq desc
         ) as event_rank
