...
| `front/shared/api/host-authority-event.test.ts` `e4975015...` plus final dashboard test `b15c2b9b...` | Task 9 full frontend gate GREEN 431 files / 3,852 tests at the unchanged parser hash; this review's focused dashboard Vitest GREEN 19/19 | A parsed 403 authority code emits one club-scoped loss event, while the mounted operating room purges that club's conflict draft, unknown state and receipt and redirects through the safe fail-closed handoff. |
